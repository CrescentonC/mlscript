

(* lumberhack_float_out *)
open Lumherhack_Common.Lumherhack_Common;;
open Lumberhack_LargeStr.Lumberhack_LargeStr;;
module Module_lumberhack_float_out_____(LH_Dum: sig end): sig val run: unit -> int end = struct
let rec lift_lh__d1 _lh_lift_arg1_0 _lh_lift_arg2_0 =
  (match _lh_lift_arg2_0 with
    | `Nothing -> 
      (`Nothing)
    | `Just(_lh_lift_Just_0_0) -> 
      (`Just((_lh_lift_arg1_0 _lh_lift_Just_0_0))));;
let rec getOrElse_lh__d1 _lh_getOrElse_arg1_0 _lh_getOrElse_arg2_0 =
  (match _lh_getOrElse_arg1_0 with
    | `Just(_lh_getOrElse_Just_0_0) -> 
      _lh_getOrElse_Just_0_0
    | `Nothing -> 
      _lh_getOrElse_arg2_0);;
let rec simplify_lh__d1 _lh_simplify_arg1_0 =
  (let rec f_1_4 = (fun t_1_1 -> 
    ((getOrElse_lh__d1 ((lift_lh__d1 f_1_4) (_lh_simplify_arg1_0 t_1_1))) t_1_1)) in
    f_1_4);;
let rec eqExpr_lh__d1 _lh_eqExpr_arg1_0 _lh_eqExpr_arg2_0 =
  (match _lh_eqExpr_arg1_0 with
    | `Func(_lh_eqExpr_Func_0_0, _lh_eqExpr_Func_1_0) -> 
      (match _lh_eqExpr_arg2_0 with
        | `Func(_lh_eqExpr_Func_0_1, _lh_eqExpr_Func_1_1) -> 
          ((_lh_eqExpr_Func_0_0 = _lh_eqExpr_Func_0_1) && ((eqListExpr_lh__d1 _lh_eqExpr_Func_1_0) _lh_eqExpr_Func_1_1))
        | _ -> 
          false)
    | `Var(_lh_eqExpr_Var_0_0) -> 
      (match _lh_eqExpr_arg2_0 with
        | `Var(_lh_eqExpr_Var_0_1) -> 
          (_lh_eqExpr_Var_0_0 = _lh_eqExpr_Var_0_1)
        | _ -> 
          false)
    | _ -> 
      false)
and
eqListExpr_lh__d1 _lh_eqListExpr_arg1_0 _lh_eqListExpr_arg2_0 =
  (match _lh_eqListExpr_arg1_0 with
    | `LH_N -> 
      (match _lh_eqListExpr_arg2_0 with
        | `LH_N -> 
          true
        | _ -> 
          false)
    | `LH_C(_lh_eqListExpr_LH_C_0_0, _lh_eqListExpr_LH_C_1_0) -> 
      (match _lh_eqListExpr_arg2_0 with
        | `LH_C(_lh_eqListExpr_LH_C_0_1, _lh_eqListExpr_LH_C_1_1) -> 
          (if ((eqExpr_lh__d1 _lh_eqListExpr_LH_C_0_0) _lh_eqListExpr_LH_C_0_1) then
            ((eqListExpr_lh__d1 _lh_eqListExpr_LH_C_1_0) _lh_eqListExpr_LH_C_1_1)
          else
            false)
        | _ -> 
          false)
    | _ -> 
      false);;
let rec rank_order_lh__d1 _lh_rank_order_arg1_0 _lh_rank_order_arg2_0 =
  (_lh_rank_order_arg2_0 _lh_rank_order_arg1_0);;
let rec the_lh__d2 _lh_the_arg1_2 =
  (match _lh_the_arg1_2 with
    | `Just(_lh_the_Just_0_2) -> 
      _lh_the_Just_0_2
    | _ -> 
      (failwith "lh_default_error"));;
let rec inList_lh__d4 _lh_inList_arg1_3 _lh_inList_arg2_0 =
  (match _lh_inList_arg2_0 with
    | `LH_C(_lh_inList_LH_C_0_2, _lh_inList_LH_C_1_2) -> 
      (if (_lh_inList_arg1_3 = _lh_inList_LH_C_0_2) then
        true
      else
        ((inList_lh__d4 _lh_inList_arg1_3) _lh_inList_LH_C_1_2))
    | `LH_N -> 
      false);;
let rec opsym_lh__d2 _lh_opsym_arg1_0 =
  ((fun x_2 -> 
    ((inList_lh__d4 x_2) (`LH_C('*', (`LH_C('+', (`LH_C('%', (`LH_C('@', (`LH_C('-', (`LH_C('/', (`LH_C('?', (`LH_C(':', (`LH_N))))))))))))))))))) _lh_opsym_arg1_0);;
let rec dropWhile_lh__d1 _lh_dropWhile_arg1_0 _lh_dropWhile_arg2_0 =
  (match _lh_dropWhile_arg2_0 with
    | `LH_N -> 
      (`LH_N)
    | `LH_C(_lh_dropWhile_LH_C_0_0, _lh_dropWhile_LH_C_1_0) -> 
      (if (_lh_dropWhile_arg1_0 _lh_dropWhile_LH_C_0_0) then
        ((dropWhile_lh__d1 _lh_dropWhile_arg1_0) _lh_dropWhile_LH_C_1_0)
      else
        (`LH_C(_lh_dropWhile_LH_C_0_0, _lh_dropWhile_LH_C_1_0))));;
let rec sp_lh__d2 _lh_sp_arg1_3 _lh_funcomp_x_9 =
  (_lh_sp_arg1_3 ((dropWhile_lh__d1 (fun x_1_6 -> 
    (x_1_6 = ' '))) _lh_funcomp_x_9));;
let rec maybeX_lh__d1 _lh_maybeX_arg1_0 _lh_maybeX_arg2_0 =
  (if _lh_maybeX_arg1_0 then
    (`Just(_lh_maybeX_arg2_0))
  else
    (`Nothing));;
let rec takeWhile_lh__d1 _lh_takeWhile_arg1_0 _lh_takeWhile_arg2_0 =
  (match _lh_takeWhile_arg2_0 with
    | `LH_N -> 
      (`LH_N)
    | `LH_C(_lh_takeWhile_LH_C_0_0, _lh_takeWhile_LH_C_1_0) -> 
      (if (_lh_takeWhile_arg1_0 _lh_takeWhile_LH_C_0_0) then
        (`LH_C(_lh_takeWhile_LH_C_0_0, ((takeWhile_lh__d1 _lh_takeWhile_arg1_0) _lh_takeWhile_LH_C_1_0)))
      else
        (`LH_N)));;
let rec string_of_lh__d2 _lh_string_of_arg1_1 _lh_string_of_arg2_1 =
  (let rec chars_1 = ((takeWhile_lh__d1 _lh_string_of_arg1_1) _lh_string_of_arg2_1) in
    (let rec s'_1 = ((dropWhile_lh__d1 _lh_string_of_arg1_1) _lh_string_of_arg2_1) in
      ((maybeX_lh__d1 (chars_1 <> (`LH_N))) (`LH_P2((`MkString(chars_1)), s'_1)))));;
let rec p_op_lh__d2 _lh_p_op_arg1_0 =
  ((sp_lh__d2 (string_of_lh__d2 opsym_lh__d2)) _lh_p_op_arg1_0);;
let rec q_op_lh__d2 _lh_q_op_arg1_0 =
  (match _lh_q_op_arg1_0 with
    | `LH_C(_lh_q_op_LH_C_0_0, _lh_q_op_LH_C_1_0) -> 
      (match _lh_q_op_LH_C_0_0 with
        | `Expr(_lh_q_op_Expr_0_0) -> 
          (match _lh_q_op_LH_C_1_0 with
            | `LH_C(_lh_q_op_LH_C_0_1, _lh_q_op_LH_C_1_1) -> 
              (match _lh_q_op_LH_C_0_1 with
                | `MkString(_lh_q_op_MkString_0_0) -> 
                  (match _lh_q_op_LH_C_1_1 with
                    | `LH_C(_lh_q_op_LH_C_0_2, _lh_q_op_LH_C_1_2) -> 
                      (match _lh_q_op_LH_C_0_2 with
                        | `Expr(_lh_q_op_Expr_0_1) -> 
                          (match _lh_q_op_LH_C_1_2 with
                            | `LH_N -> 
                              (`Expr((`Func(_lh_q_op_MkString_0_0, (`LH_C(_lh_q_op_Expr_0_0, (`LH_C(_lh_q_op_Expr_0_1, (`LH_N)))))))))
                            | _ -> 
                              (failwith "lh_default_error"))
                        | _ -> 
                          (failwith "lh_default_error"))
                    | _ -> 
                      (failwith "lh_default_error"))
                | _ -> 
                  (failwith "lh_default_error"))
            | _ -> 
              (failwith "lh_default_error"))
        | _ -> 
          (failwith "lh_default_error"))
    | _ -> 
      (failwith "lh_default_error"));;
let rec foldr_lh__d6 f_2_2 i_2_0 ls_2 =
  ((ls_2 f_2_2) i_2_0);;
let rec unExpr_lh__d3 _lh_unExpr_arg1_2 =
  (match _lh_unExpr_arg1_2 with
    | `Expr(_lh_unExpr_Expr_0_2) -> 
      _lh_unExpr_Expr_0_2
    | _ -> 
      (failwith "lh_default_error"));;
let rec map_lh__d7 f_2_1 ls_1 =
  (match ls_1 with
    | `LH_C(h_1_5, t_1_7) -> 
      (`LH_C((f_2_1 h_1_5), ((map_lh__d7 f_2_1) t_1_7)))
    | `LH_N -> 
      (`LH_N));;
let rec q_func_lh__d6 _lh_q_func_arg1_2 =
  (match _lh_q_func_arg1_2 with
    | `LH_C(_lh_q_func_LH_C_0_8, _lh_q_func_LH_C_1_8) -> 
      (match _lh_q_func_LH_C_0_8 with
        | `MkString(_lh_q_func_MkString_0_2) -> 
          (match _lh_q_func_LH_C_1_8 with
            | `LH_C(_lh_q_func_LH_C_0_9, _lh_q_func_LH_C_1_9) -> 
              (match _lh_q_func_LH_C_1_9 with
                | `LH_C(_lh_q_func_LH_C_0_1_0, _lh_q_func_LH_C_1_1_0) -> 
                  (match _lh_q_func_LH_C_0_1_0 with
                    | `List(_lh_q_func_List_0_2) -> 
                      (match _lh_q_func_LH_C_1_1_0 with
                        | `LH_C(_lh_q_func_LH_C_0_1_1, _lh_q_func_LH_C_1_1_1) -> 
                          (match _lh_q_func_LH_C_1_1_1 with
                            | `LH_N -> 
                              (`Expr((`Func(_lh_q_func_MkString_0_2, ((map_lh__d7 unExpr_lh__d3) _lh_q_func_List_0_2)))))
                            | _ -> 
                              (failwith "lh_default_error"))
                        | _ -> 
                          (failwith "lh_default_error"))
                    | _ -> 
                      (failwith "lh_default_error"))
                | _ -> 
                  (failwith "lh_default_error"))
            | _ -> 
              (failwith "lh_default_error"))
        | _ -> 
          (failwith "lh_default_error"))
    | _ -> 
      (failwith "lh_default_error"));;
let rec orElse_lh__d6 _lh_orElse_arg1_5 _lh_orElse_arg2_5 =
  (match _lh_orElse_arg1_5 with
    | `Nothing -> 
      _lh_orElse_arg2_5
    | _ -> 
      _lh_orElse_arg1_5);;
let rec orElseMap_lh__d1_d5 _lh_orElseMap_arg1_5 _lh_orElseMap_arg2_5 _lh_orElseMap_arg3_5 =
  ((orElse_lh__d6 (_lh_orElseMap_arg1_5 _lh_orElseMap_arg3_5)) (_lh_orElseMap_arg2_5 _lh_orElseMap_arg3_5));;
let rec sp_lh__d8 _lh_sp_arg1_6 _lh_funcomp_x_2_4 =
  (_lh_sp_arg1_6 ((dropWhile_lh__d1 (fun x_3_0 -> 
    (x_3_0 = ' '))) _lh_funcomp_x_2_4));;
let rec succeed_lh__d1 _lh_succeed_arg1_0 =
  (`Just(_lh_succeed_arg1_0));;
let rec croak_lh__d1 =
  (`Nothing);;
let rec pchar_lh__d4 _lh_pchar_arg1_1 _lh_pchar_arg2_1 =
  (match _lh_pchar_arg2_1 with
    | `LH_N -> 
      croak_lh__d1
    | `LH_C(_lh_pchar_LH_C_0_1, _lh_pchar_LH_C_1_1) -> 
      (if (_lh_pchar_arg1_1 _lh_pchar_LH_C_0_1) then
        (succeed_lh__d1 (`LH_P2((`MkString((`LH_C(_lh_pchar_LH_C_0_1, (`LH_N))))), _lh_pchar_LH_C_1_1)))
      else
        croak_lh__d1));;
let rec look_for_lh__d1_d9 _lh_look_for_arg1_4 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_2_5 -> 
    (x_2_5 = _lh_look_for_arg1_4))));;
let rec empty_lh__d6 _lh_empty_arg1_1 =
  (succeed_lh__d1 (`LH_P2((`List((`LH_N))), _lh_empty_arg1_1)));;
let rec lift_lh__d6 _lh_lift_arg1_5 _lh_lift_arg2_5 =
  (match _lh_lift_arg2_5 with
    | `Nothing -> 
      (`Nothing)
    | `Just(_lh_lift_Just_0_5) -> 
      (`Just((_lh_lift_arg1_5 _lh_lift_Just_0_5))));;
let rec squash_lh__d1 _lh_squash_arg1_0 =
  (match _lh_squash_arg1_0 with
    | `Just(_lh_squash_Just_0_0) -> 
      _lh_squash_Just_0_0
    | `Nothing -> 
      (`Nothing));;
let rec cross_lh__d2 _lh_cross_arg1_0 _lh_cross_arg2_0 _lh_cross_arg3_0 =
  (match _lh_cross_arg3_0 with
    | `LH_P2(_lh_cross_LH_P2_0_0, _lh_cross_LH_P2_1_0) -> 
      (`LH_P2((_lh_cross_arg1_0 _lh_cross_LH_P2_0_0), (_lh_cross_arg2_0 _lh_cross_LH_P2_1_0))));;
let rec seq2_lh__d7 _lh_seq2_arg1_4 _lh_seq2_arg2_4 _lh_seq2_arg3_4 _lh_funcomp_x_2_8 =
  (let rec g_5 = (fun xs_8 -> 
    (let rec _lh_matchIdent_8 = xs_8 in
      (match _lh_matchIdent_8 with
        | `LH_P2(_lh_seq2_LH_P2_0_4, _lh_seq2_LH_P2_1_4) -> 
          ((lift_lh__d6 ((cross_lh__d2 (_lh_seq2_arg1_4 _lh_seq2_LH_P2_0_4)) (fun x_4_4 -> 
            x_4_4))) (_lh_seq2_arg3_4 _lh_seq2_LH_P2_1_4))))) in
    ((fun _lh_funcomp_x_2_9 -> 
      (squash_lh__d1 ((lift_lh__d6 g_5) _lh_funcomp_x_2_9))) (_lh_seq2_arg2_4 _lh_funcomp_x_2_8)));;
let rec mk_cons_lh__d7 _lh_mk_cons_arg1_6 _lh_mk_cons_arg2_6 =
  (match _lh_mk_cons_arg2_6 with
    | `List(_lh_mk_cons_List_0_6) -> 
      (`List((`LH_C(_lh_mk_cons_arg1_6, _lh_mk_cons_List_0_6))))
    | _ -> 
      (failwith "lh_default_error"));;
let rec build_lh__d5 _lh_build_arg1_4 _lh_build_arg2_4 _lh_funcomp_x_3_2 =
  ((lift_lh__d6 ((cross_lh__d2 _lh_build_arg1_4) (fun x_6_2 -> 
    x_6_2))) (_lh_build_arg2_4 _lh_funcomp_x_3_2));;
let rec mk_list_lh__d5 _lh_mk_list_arg1_4 _lh_mk_list_arg2_4 =
  (match _lh_mk_list_arg2_4 with
    | `List(_lh_mk_list_List_0_4) -> 
      (_lh_mk_list_arg1_4 _lh_mk_list_List_0_4)
    | _ -> 
      (failwith "lh_default_error"));;
let rec seQ_lh__d1_d3 _lh_seQ_arg1_7 _lh_seQ_arg2_7 =
  ((build_lh__d5 (mk_list_lh__d5 _lh_seQ_arg1_7)) (((foldr_lh__d6 (seq2_lh__d7 mk_cons_lh__d7)) empty_lh__d6) _lh_seQ_arg2_7));;
let rec atIndex_lh__d2 n_0 ls_1_9 =
  (if (n_0 < 0) then
    (failwith "lh_default_error")
  else
    (match ls_1_9 with
      | `LH_C(h_9_9, t_1_0_2) -> 
        (if (n_0 = 0) then
          h_9_9
        else
          ((atIndex_lh__d2 (n_0 - 1)) t_1_0_2))
      | `LH_N -> 
        (failwith "lh_default_error")));;
let rec string_of_lh__d4 _lh_string_of_arg1_2 _lh_string_of_arg2_2 =
  (let rec chars_2 = ((takeWhile_lh__d1 _lh_string_of_arg1_2) _lh_string_of_arg2_2) in
    (let rec s'_2 = ((dropWhile_lh__d1 _lh_string_of_arg1_2) _lh_string_of_arg2_2) in
      ((maybeX_lh__d1 (chars_2 <> (`LH_N))) (`LH_P2((`MkString(chars_2)), s'_2)))));;
let rec alphanum_lh__d2 _lh_alphanum_arg1_0 =
  (((((int_of_char _lh_alphanum_arg1_0) >= (int_of_char 'A')) && ((int_of_char _lh_alphanum_arg1_0) <= (int_of_char 'Z'))) || (((int_of_char _lh_alphanum_arg1_0) >= (int_of_char 'a')) && ((int_of_char _lh_alphanum_arg1_0) <= (int_of_char 'z')))) || (((int_of_char _lh_alphanum_arg1_0) >= (int_of_char '0')) && ((int_of_char _lh_alphanum_arg1_0) <= (int_of_char '9'))));;
let rec p_ident_lh__d6 _lh_p_ident_arg1_4 =
  ((sp_lh__d8 (string_of_lh__d4 alphanum_lh__d2)) _lh_p_ident_arg1_4);;
let rec q_name_lh__d1 _lh_q_name_arg1_0 =
  (match _lh_q_name_arg1_0 with
    | `MkString(_lh_q_name_MkString_0_0) -> 
      (if (((int_of_char ((atIndex_lh__d2 0) _lh_q_name_MkString_0_0)) >= (int_of_char 'a')) && ((int_of_char ((atIndex_lh__d2 0) _lh_q_name_MkString_0_0)) <= (int_of_char 'z'))) then
        (`Expr((`Var(_lh_q_name_MkString_0_0))))
      else
        (`Expr((`Func(_lh_q_name_MkString_0_0, (`LH_N))))))
    | _ -> 
      (failwith "lh_default_error"));;
let rec p_name_lh__d2 _lh_p_name_arg1_0 =
  (((build_lh__d5 q_name_lh__d1) p_ident_lh__d6) _lh_p_name_arg1_0);;
let rec seQ_lh__d9 _lh_seQ_arg1_6 _lh_seQ_arg2_6 =
  ((build_lh__d5 (mk_list_lh__d5 _lh_seQ_arg1_6)) (((foldr_lh__d6 (seq2_lh__d7 mk_cons_lh__d7)) empty_lh__d6) _lh_seQ_arg2_6));;
let rec seq2Lzq_lh__d2 _lh_seq2Lzq_arg1_0 _lh_seq2Lzq_arg2_0 _lh_seq2Lzq_arg3_0 _lh_funcomp_x_1_2 =
  (let rec g_2 = (fun xs_4 -> 
    (let rec _lh_matchIdent_2 = xs_4 in
      (match _lh_matchIdent_2 with
        | `LH_P2(_lh_seq2Lzq_LH_P2_0_0, _lh_seq2Lzq_LH_P2_1_0) -> 
          ((lift_lh__d6 ((cross_lh__d2 (_lh_seq2Lzq_arg1_0 _lh_seq2Lzq_LH_P2_0_0)) (fun x_2_1 -> 
            x_2_1))) ((Lazy.force _lh_seq2Lzq_arg3_0) _lh_seq2Lzq_LH_P2_1_0))))) in
    ((fun _lh_funcomp_x_1_3 -> 
      (squash_lh__d1 ((lift_lh__d6 g_2) _lh_funcomp_x_1_3))) (_lh_seq2Lzq_arg2_0 _lh_funcomp_x_1_2)));;
let rec list_of_lh__d6 _lh_list_of_arg1_5 _lh_list_of_arg2_5 =
  (let rec p'_5 = (lazy (((seq2_lh__d7 mk_cons_lh__d7) _lh_list_of_arg1_5) ((orElseMap_lh__d1_d5 (((seq2Lzq_lh__d2 (fun x_5_5 y_7 -> 
    y_7)) (look_for_lh__d1_d9 _lh_list_of_arg2_5)) p'_5)) empty_lh__d6))) in
    (Lazy.force p'_5));;
let rec look_for_lh__d1_d2 _lh_look_for_arg1_1_5 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_5_8 -> 
    (x_5_8 = _lh_look_for_arg1_1_5))));;
let rec look_for_lh__d1_d3 _lh_look_for_arg1_1_7 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_6_4 -> 
    (x_6_4 = _lh_look_for_arg1_1_7))));;
let rec orElseMap_lh__d9 _lh_orElseMap_arg1_1_2 _lh_orElseMap_arg2_1_2 _lh_orElseMap_arg3_1_2 =
  ((orElse_lh__d6 (_lh_orElseMap_arg1_1_2 _lh_orElseMap_arg3_1_2)) (_lh_orElseMap_arg2_1_2 _lh_orElseMap_arg3_1_2));;
let rec build_lh__d2 _lh_build_arg1_0 _lh_build_arg2_0 _lh_funcomp_x_1 =
  ((lift_lh__d6 ((cross_lh__d2 _lh_build_arg1_0) (fun x_1 -> 
    x_1))) (_lh_build_arg2_0 _lh_funcomp_x_1));;
let rec mk_list_lh__d2 _lh_mk_list_arg1_1 _lh_mk_list_arg2_1 =
  (match _lh_mk_list_arg2_1 with
    | `List(_lh_mk_list_List_0_1) -> 
      (_lh_mk_list_arg1_1 _lh_mk_list_List_0_1)
    | _ -> 
      (failwith "lh_default_error"));;
let rec empty_lh__d2 _lh_empty_arg1_3 =
  (succeed_lh__d1 (`LH_P2((`List((`LH_N))), _lh_empty_arg1_3)));;
let rec mk_cons_lh__d2 _lh_mk_cons_arg1_3 _lh_mk_cons_arg2_3 =
  (match _lh_mk_cons_arg2_3 with
    | `List(_lh_mk_cons_List_0_3) -> 
      (`List((`LH_C(_lh_mk_cons_arg1_3, _lh_mk_cons_List_0_3))))
    | _ -> 
      (failwith "lh_default_error"));;
let rec foldr_lh__d4 f_8_4 i_7_7 ls_9 =
  ((ls_9 f_8_4) i_7_7);;
let rec seq2_lh__d2 _lh_seq2_arg1_5 _lh_seq2_arg2_5 _lh_seq2_arg3_5 _lh_funcomp_x_3_0 =
  (let rec g_6 = (fun xs_9 -> 
    (let rec _lh_matchIdent_1_3 = xs_9 in
      (match _lh_matchIdent_1_3 with
        | `LH_P2(_lh_seq2_LH_P2_0_5, _lh_seq2_LH_P2_1_5) -> 
          ((lift_lh__d6 ((cross_lh__d2 (_lh_seq2_arg1_5 _lh_seq2_LH_P2_0_5)) (fun x_4_8 -> 
            x_4_8))) (_lh_seq2_arg3_5 _lh_seq2_LH_P2_1_5))))) in
    ((fun _lh_funcomp_x_3_1 -> 
      (squash_lh__d1 ((lift_lh__d6 g_6) _lh_funcomp_x_3_1))) (_lh_seq2_arg2_5 _lh_funcomp_x_3_0)));;
let rec seQ_lh__d2 _lh_seQ_arg1_5 _lh_seQ_arg2_5 =
  ((build_lh__d2 (mk_list_lh__d2 _lh_seQ_arg1_5)) (((foldr_lh__d4 (seq2_lh__d2 mk_cons_lh__d2)) empty_lh__d2) _lh_seQ_arg2_5));;
let rec p_ident_lh__d4 _lh_p_ident_arg1_1 =
  ((sp_lh__d8 (string_of_lh__d4 alphanum_lh__d2)) _lh_p_ident_arg1_1);;
let rec look_for_lh__d9 _lh_look_for_arg1_8 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_4_3 -> 
    (x_4_3 = _lh_look_for_arg1_8))));;
let rec look_for_lh__d8 _lh_look_for_arg1_1_0 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_5_1 -> 
    (x_5_1 = _lh_look_for_arg1_1_0))));;
let rec orElseMap_lh__d7 _lh_orElseMap_arg1_9 _lh_orElseMap_arg2_9 _lh_orElseMap_arg3_9 =
  ((orElse_lh__d6 (_lh_orElseMap_arg1_9 _lh_orElseMap_arg3_9)) (_lh_orElseMap_arg2_9 _lh_orElseMap_arg3_9));;
let rec q_func_lh__d4 _lh_q_func_arg1_5 =
  (match _lh_q_func_arg1_5 with
    | `LH_C(_lh_q_func_LH_C_0_2_0, _lh_q_func_LH_C_1_2_0) -> 
      (match _lh_q_func_LH_C_0_2_0 with
        | `MkString(_lh_q_func_MkString_0_5) -> 
          (match _lh_q_func_LH_C_1_2_0 with
            | `LH_C(_lh_q_func_LH_C_0_2_1, _lh_q_func_LH_C_1_2_1) -> 
              (match _lh_q_func_LH_C_1_2_1 with
                | `LH_C(_lh_q_func_LH_C_0_2_2, _lh_q_func_LH_C_1_2_2) -> 
                  (match _lh_q_func_LH_C_0_2_2 with
                    | `List(_lh_q_func_List_0_5) -> 
                      (match _lh_q_func_LH_C_1_2_2 with
                        | `LH_C(_lh_q_func_LH_C_0_2_3, _lh_q_func_LH_C_1_2_3) -> 
                          (match _lh_q_func_LH_C_1_2_3 with
                            | `LH_N -> 
                              (`Expr((`Func(_lh_q_func_MkString_0_5, ((map_lh__d7 unExpr_lh__d3) _lh_q_func_List_0_5)))))
                            | _ -> 
                              (failwith "lh_default_error"))
                        | _ -> 
                          (failwith "lh_default_error"))
                    | _ -> 
                      (failwith "lh_default_error"))
                | _ -> 
                  (failwith "lh_default_error"))
            | _ -> 
              (failwith "lh_default_error"))
        | _ -> 
          (failwith "lh_default_error"))
    | _ -> 
      (failwith "lh_default_error"));;
let rec seQ_lh__d7 _lh_seQ_arg1_0 _lh_seQ_arg2_0 =
  ((build_lh__d5 (mk_list_lh__d5 _lh_seQ_arg1_0)) (((foldr_lh__d6 (seq2_lh__d7 mk_cons_lh__d7)) empty_lh__d6) _lh_seQ_arg2_0));;
let rec list_of_lh__d4 _lh_list_of_arg1_1 _lh_list_of_arg2_1 =
  (let rec p'_1 = (lazy (((seq2_lh__d7 mk_cons_lh__d7) _lh_list_of_arg1_1) ((orElseMap_lh__d1_d5 (((seq2Lzq_lh__d2 (fun x_1_9 y_1 -> 
    y_1)) (look_for_lh__d1_d9 _lh_list_of_arg2_1)) p'_1)) empty_lh__d6))) in
    (Lazy.force p'_1));;
let rec orElseMap_lh__d8 _lh_orElseMap_arg1_3 _lh_orElseMap_arg2_3 _lh_orElseMap_arg3_3 =
  ((orElse_lh__d6 (_lh_orElseMap_arg1_3 _lh_orElseMap_arg3_3)) (_lh_orElseMap_arg2_3 _lh_orElseMap_arg3_3));;
let rec look_for_lh__d1_d1 _lh_look_for_arg1_2 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_1_3 -> 
    (x_1_3 = _lh_look_for_arg1_2))));;
let rec look_for_lh__d1_d0 _lh_look_for_arg1_1_1 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_5_2 -> 
    (x_5_2 = _lh_look_for_arg1_1_1))));;
let rec q_func_lh__d5 _lh_q_func_arg1_3 =
  (match _lh_q_func_arg1_3 with
    | `LH_C(_lh_q_func_LH_C_0_1_2, _lh_q_func_LH_C_1_1_2) -> 
      (match _lh_q_func_LH_C_0_1_2 with
        | `MkString(_lh_q_func_MkString_0_3) -> 
          (match _lh_q_func_LH_C_1_1_2 with
            | `LH_C(_lh_q_func_LH_C_0_1_3, _lh_q_func_LH_C_1_1_3) -> 
              (match _lh_q_func_LH_C_1_1_3 with
                | `LH_C(_lh_q_func_LH_C_0_1_4, _lh_q_func_LH_C_1_1_4) -> 
                  (match _lh_q_func_LH_C_0_1_4 with
                    | `List(_lh_q_func_List_0_3) -> 
                      (match _lh_q_func_LH_C_1_1_4 with
                        | `LH_C(_lh_q_func_LH_C_0_1_5, _lh_q_func_LH_C_1_1_5) -> 
                          (match _lh_q_func_LH_C_1_1_5 with
                            | `LH_N -> 
                              (`Expr((`Func(_lh_q_func_MkString_0_3, ((map_lh__d7 unExpr_lh__d3) _lh_q_func_List_0_3)))))
                            | _ -> 
                              (failwith "lh_default_error"))
                        | _ -> 
                          (failwith "lh_default_error"))
                    | _ -> 
                      (failwith "lh_default_error"))
                | _ -> 
                  (failwith "lh_default_error"))
            | _ -> 
              (failwith "lh_default_error"))
        | _ -> 
          (failwith "lh_default_error"))
    | _ -> 
      (failwith "lh_default_error"));;
let rec seQ_lh__d8 _lh_seQ_arg1_1_2 _lh_seQ_arg2_1_2 =
  ((build_lh__d5 (mk_list_lh__d5 _lh_seQ_arg1_1_2)) (((foldr_lh__d6 (seq2_lh__d7 mk_cons_lh__d7)) empty_lh__d6) _lh_seQ_arg2_1_2));;
let rec p_ident_lh__d5 _lh_p_ident_arg1_3 =
  ((sp_lh__d8 (string_of_lh__d4 alphanum_lh__d2)) _lh_p_ident_arg1_3);;
let rec list_of_lh__d5 _lh_list_of_arg1_2 _lh_list_of_arg2_2 =
  (let rec p'_2 = (lazy (((seq2_lh__d7 mk_cons_lh__d7) _lh_list_of_arg1_2) ((orElseMap_lh__d1_d5 (((seq2Lzq_lh__d2 (fun x_3_4 y_2 -> 
    y_2)) (look_for_lh__d1_d9 _lh_list_of_arg2_2)) p'_2)) empty_lh__d6))) in
    (Lazy.force p'_2));;
let rec orElse_lh__d3 _lh_orElse_arg1_0 _lh_orElse_arg2_0 =
  (match _lh_orElse_arg1_0 with
    | `Nothing -> 
      _lh_orElse_arg2_0
    | _ -> 
      _lh_orElse_arg1_0);;
let rec orElseMap_lh__d2 _lh_orElseMap_arg1_1_3 _lh_orElseMap_arg2_1_3 _lh_orElseMap_arg3_1_3 =
  ((orElse_lh__d3 (_lh_orElseMap_arg1_1_3 _lh_orElseMap_arg3_1_3)) (_lh_orElseMap_arg2_1_3 _lh_orElseMap_arg3_1_3));;
let rec p_expr_lh__d2 _lh_p_expr_arg1_1 =
  (((orElseMap_lh__d2 ((seQ_lh__d2 q_op_lh__d2) (let rec t_3_1 = (let rec t_3_2 = (let rec t_3_3 = (fun f_4_0 i_3_4 -> 
    i_3_4) in
    (let rec h_2_9 = p_term_lh__d6 in
      (fun f_4_1 i_3_5 -> 
        ((f_4_1 h_2_9) (((foldr_lh__d4 f_4_1) i_3_5) t_3_3))))) in
    (let rec h_3_0 = p_op_lh__d2 in
      (fun f_4_2 i_3_6 -> 
        ((f_4_2 h_3_0) (((foldr_lh__d4 f_4_2) i_3_6) t_3_2))))) in
    (let rec h_3_1 = p_term_lh__d4 in
      (fun f_4_3 i_3_7 -> 
        ((f_4_3 h_3_1) (((foldr_lh__d4 f_4_3) i_3_7) t_3_1))))))) p_term_lh__d5) _lh_p_expr_arg1_1)
and
p_prim_lh__d4 _lh_p_prim_arg1_0 =
  (((orElseMap_lh__d1_d5 p_name_lh__d2) ((seQ_lh__d1_d3 (fun x_4 -> 
    ((atIndex_lh__d2 1) x_4))) (let rec t_8 = (let rec t_9 = (let rec t_1_0 = (fun f_1_0 i_1_0 -> 
    i_1_0) in
    (let rec h_8 = (look_for_lh__d1_d9 ')') in
      (fun f_1_1 i_1_1 -> 
        ((f_1_1 h_8) (((foldr_lh__d6 f_1_1) i_1_1) t_1_0))))) in
    (let rec h_9 = p_expr_lh__d2 in
      (fun f_1_2 i_1_2 -> 
        ((f_1_2 h_9) (((foldr_lh__d6 f_1_2) i_1_2) t_9))))) in
    (let rec h_1_0 = (look_for_lh__d1_d9 '(') in
      (fun f_1_3 i_1_3 -> 
        ((f_1_3 h_1_0) (((foldr_lh__d6 f_1_3) i_1_3) t_8))))))) _lh_p_prim_arg1_0)
and
p_prim_lh__d5 _lh_p_prim_arg1_5 =
  (((orElseMap_lh__d1_d5 p_name_lh__d2) ((seQ_lh__d1_d3 (fun x_6_3 -> 
    ((atIndex_lh__d2 1) x_6_3))) (let rec t_9_0 = (let rec t_9_1 = (let rec t_9_2 = (fun f_1_1_3 i_1_0_0 -> 
    i_1_0_0) in
    (let rec h_8_7 = (look_for_lh__d1_d9 ')') in
      (fun f_1_1_4 i_1_0_1 -> 
        ((f_1_1_4 h_8_7) (((foldr_lh__d6 f_1_1_4) i_1_0_1) t_9_2))))) in
    (let rec h_8_8 = p_expr_lh__d2 in
      (fun f_1_1_5 i_1_0_2 -> 
        ((f_1_1_5 h_8_8) (((foldr_lh__d6 f_1_1_5) i_1_0_2) t_9_1))))) in
    (let rec h_8_9 = (look_for_lh__d1_d9 '(') in
      (fun f_1_1_6 i_1_0_3 -> 
        ((f_1_1_6 h_8_9) (((foldr_lh__d6 f_1_1_6) i_1_0_3) t_9_0))))))) _lh_p_prim_arg1_5)
and
p_prim_lh__d6 _lh_p_prim_arg1_4 =
  (((orElseMap_lh__d1_d5 p_name_lh__d2) ((seQ_lh__d1_d3 (fun x_3_9 -> 
    ((atIndex_lh__d2 1) x_3_9))) (let rec t_6_7 = (let rec t_6_8 = (let rec t_6_9 = (fun f_8_5 i_7_8 -> 
    i_7_8) in
    (let rec h_6_4 = (look_for_lh__d1_d9 ')') in
      (fun f_8_6 i_7_9 -> 
        ((f_8_6 h_6_4) (((foldr_lh__d6 f_8_6) i_7_9) t_6_9))))) in
    (let rec h_6_5 = p_expr_lh__d2 in
      (fun f_8_7 i_8_0 -> 
        ((f_8_7 h_6_5) (((foldr_lh__d6 f_8_7) i_8_0) t_6_8))))) in
    (let rec h_6_6 = (look_for_lh__d1_d9 '(') in
      (fun f_8_8 i_8_1 -> 
        ((f_8_8 h_6_6) (((foldr_lh__d6 f_8_8) i_8_1) t_6_7))))))) _lh_p_prim_arg1_4)
and
p_term_lh__d4 _lh_p_term_arg1_7 =
  (((orElseMap_lh__d7 ((seQ_lh__d7 q_func_lh__d4) (let rec t_6_3 = (let rec t_6_4 = (let rec t_6_5 = (let rec t_6_6 = (fun f_7_9 i_7_2 -> 
    i_7_2) in
    (let rec h_6_0 = (look_for_lh__d9 ')') in
      (fun f_8_0 i_7_3 -> 
        ((f_8_0 h_6_0) (((foldr_lh__d6 f_8_0) i_7_3) t_6_6))))) in
    (let rec h_6_1 = ((list_of_lh__d4 p_expr_lh__d2) ',') in
      (fun f_8_1 i_7_4 -> 
        ((f_8_1 h_6_1) (((foldr_lh__d6 f_8_1) i_7_4) t_6_5))))) in
    (let rec h_6_2 = (look_for_lh__d8 '(') in
      (fun f_8_2 i_7_5 -> 
        ((f_8_2 h_6_2) (((foldr_lh__d6 f_8_2) i_7_5) t_6_4))))) in
    (let rec h_6_3 = p_ident_lh__d4 in
      (fun f_8_3 i_7_6 -> 
        ((f_8_3 h_6_3) (((foldr_lh__d6 f_8_3) i_7_6) t_6_3))))))) p_prim_lh__d4) _lh_p_term_arg1_7)
and
p_term_lh__d5 _lh_p_term_arg1_8 =
  (((orElseMap_lh__d8 ((seQ_lh__d8 q_func_lh__d5) (let rec t_7_3 = (let rec t_7_4 = (let rec t_7_5 = (let rec t_7_6 = (fun f_9_3 i_8_4 -> 
    i_8_4) in
    (let rec h_7_0 = (look_for_lh__d1_d1 ')') in
      (fun f_9_4 i_8_5 -> 
        ((f_9_4 h_7_0) (((foldr_lh__d6 f_9_4) i_8_5) t_7_6))))) in
    (let rec h_7_1 = ((list_of_lh__d5 p_expr_lh__d2) ',') in
      (fun f_9_5 i_8_6 -> 
        ((f_9_5 h_7_1) (((foldr_lh__d6 f_9_5) i_8_6) t_7_5))))) in
    (let rec h_7_2 = (look_for_lh__d1_d0 '(') in
      (fun f_9_6 i_8_7 -> 
        ((f_9_6 h_7_2) (((foldr_lh__d6 f_9_6) i_8_7) t_7_4))))) in
    (let rec h_7_3 = p_ident_lh__d5 in
      (fun f_9_7 i_8_8 -> 
        ((f_9_7 h_7_3) (((foldr_lh__d6 f_9_7) i_8_8) t_7_3))))))) p_prim_lh__d5) _lh_p_term_arg1_8)
and
p_term_lh__d6 _lh_p_term_arg1_2 =
  (((orElseMap_lh__d9 ((seQ_lh__d9 q_func_lh__d6) (let rec t_1_3 = (let rec t_1_4 = (let rec t_1_5 = (let rec t_1_6 = (fun f_1_6 i_1_5 -> 
    i_1_5) in
    (let rec h_1_1 = (look_for_lh__d1_d3 ')') in
      (fun f_1_7 i_1_6 -> 
        ((f_1_7 h_1_1) (((foldr_lh__d6 f_1_7) i_1_6) t_1_6))))) in
    (let rec h_1_2 = ((list_of_lh__d6 p_expr_lh__d2) ',') in
      (fun f_1_8 i_1_7 -> 
        ((f_1_8 h_1_2) (((foldr_lh__d6 f_1_8) i_1_7) t_1_5))))) in
    (let rec h_1_3 = (look_for_lh__d1_d2 '(') in
      (fun f_1_9 i_1_8 -> 
        ((f_1_9 h_1_3) (((foldr_lh__d6 f_1_9) i_1_8) t_1_4))))) in
    (let rec h_1_4 = p_ident_lh__d6 in
      (fun f_2_0 i_1_9 -> 
        ((f_2_0 h_1_4) (((foldr_lh__d6 f_2_0) i_1_9) t_1_3))))))) p_prim_lh__d6) _lh_p_term_arg1_2);;
let rec unExpr_lh__d2 _lh_unExpr_arg1_0 =
  (match _lh_unExpr_arg1_0 with
    | `Expr(_lh_unExpr_Expr_0_0) -> 
      _lh_unExpr_Expr_0_0
    | _ -> 
      (failwith "lh_default_error"));;
let rec fst_lh__d2 _lh_fst_arg1_0 =
  (match _lh_fst_arg1_0 with
    | `LH_P2(_lh_fst_LH_P2_0_0, _lh_fst_LH_P2_1_0) -> 
      _lh_fst_LH_P2_0_0);;
let rec parse_lh__d2 _lh_parse_arg1_0 =
  ((fun _lh_funcomp_x_1_4 -> 
    ((fun _lh_funcomp_x_1_5 -> 
      ((fun _lh_funcomp_x_1_6 -> 
        (unExpr_lh__d2 (fst_lh__d2 _lh_funcomp_x_1_6))) (the_lh__d2 _lh_funcomp_x_1_5))) (p_expr_lh__d2 _lh_funcomp_x_1_4))) _lh_parse_arg1_0);;
let rec unExpr_lh__d1 _lh_unExpr_arg1_1 =
  (match _lh_unExpr_arg1_1 with
    | `Expr(_lh_unExpr_Expr_0_1) -> 
      _lh_unExpr_Expr_0_1
    | _ -> 
      (failwith "lh_default_error"));;
let rec the_lh__d1 _lh_the_arg1_1 =
  (match _lh_the_arg1_1 with
    | `Just(_lh_the_Just_0_1) -> 
      _lh_the_Just_0_1
    | _ -> 
      (failwith "lh_default_error"));;
let rec p_ident_lh__d3 _lh_p_ident_arg1_0 =
  ((sp_lh__d8 (string_of_lh__d4 alphanum_lh__d2)) _lh_p_ident_arg1_0);;
let rec look_for_lh__d7 _lh_look_for_arg1_5 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_3_2 -> 
    (x_3_2 = _lh_look_for_arg1_5))));;
let rec list_of_lh__d3 _lh_list_of_arg1_4 _lh_list_of_arg2_4 =
  (let rec p'_4 = (lazy (((seq2_lh__d7 mk_cons_lh__d7) _lh_list_of_arg1_4) ((orElseMap_lh__d1_d5 (((seq2Lzq_lh__d2 (fun x_5_4 y_6 -> 
    y_6)) (look_for_lh__d1_d9 _lh_list_of_arg2_4)) p'_4)) empty_lh__d6))) in
    (Lazy.force p'_4));;
let rec seQ_lh__d6 _lh_seQ_arg1_9 _lh_seQ_arg2_9 =
  ((build_lh__d5 (mk_list_lh__d5 _lh_seQ_arg1_9)) (((foldr_lh__d6 (seq2_lh__d7 mk_cons_lh__d7)) empty_lh__d6) _lh_seQ_arg2_9));;
let rec look_for_lh__d6 _lh_look_for_arg1_1_8 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_6_5 -> 
    (x_6_5 = _lh_look_for_arg1_1_8))));;
let rec q_func_lh__d3 _lh_q_func_arg1_4 =
  (match _lh_q_func_arg1_4 with
    | `LH_C(_lh_q_func_LH_C_0_1_6, _lh_q_func_LH_C_1_1_6) -> 
      (match _lh_q_func_LH_C_0_1_6 with
        | `MkString(_lh_q_func_MkString_0_4) -> 
          (match _lh_q_func_LH_C_1_1_6 with
            | `LH_C(_lh_q_func_LH_C_0_1_7, _lh_q_func_LH_C_1_1_7) -> 
              (match _lh_q_func_LH_C_1_1_7 with
                | `LH_C(_lh_q_func_LH_C_0_1_8, _lh_q_func_LH_C_1_1_8) -> 
                  (match _lh_q_func_LH_C_0_1_8 with
                    | `List(_lh_q_func_List_0_4) -> 
                      (match _lh_q_func_LH_C_1_1_8 with
                        | `LH_C(_lh_q_func_LH_C_0_1_9, _lh_q_func_LH_C_1_1_9) -> 
                          (match _lh_q_func_LH_C_1_1_9 with
                            | `LH_N -> 
                              (`Expr((`Func(_lh_q_func_MkString_0_4, ((map_lh__d7 unExpr_lh__d3) _lh_q_func_List_0_4)))))
                            | _ -> 
                              (failwith "lh_default_error"))
                        | _ -> 
                          (failwith "lh_default_error"))
                    | _ -> 
                      (failwith "lh_default_error"))
                | _ -> 
                  (failwith "lh_default_error"))
            | _ -> 
              (failwith "lh_default_error"))
        | _ -> 
          (failwith "lh_default_error"))
    | _ -> 
      (failwith "lh_default_error"));;
let rec orElseMap_lh__d6 _lh_orElseMap_arg1_1_4 _lh_orElseMap_arg2_1_4 _lh_orElseMap_arg3_1_4 =
  ((orElse_lh__d6 (_lh_orElseMap_arg1_1_4 _lh_orElseMap_arg3_1_4)) (_lh_orElseMap_arg2_1_4 _lh_orElseMap_arg3_1_4));;
let rec q_func_lh__d2 _lh_q_func_arg1_0 =
  (match _lh_q_func_arg1_0 with
    | `LH_C(_lh_q_func_LH_C_0_0, _lh_q_func_LH_C_1_0) -> 
      (match _lh_q_func_LH_C_0_0 with
        | `MkString(_lh_q_func_MkString_0_0) -> 
          (match _lh_q_func_LH_C_1_0 with
            | `LH_C(_lh_q_func_LH_C_0_1, _lh_q_func_LH_C_1_1) -> 
              (match _lh_q_func_LH_C_1_1 with
                | `LH_C(_lh_q_func_LH_C_0_2, _lh_q_func_LH_C_1_2) -> 
                  (match _lh_q_func_LH_C_0_2 with
                    | `List(_lh_q_func_List_0_0) -> 
                      (match _lh_q_func_LH_C_1_2 with
                        | `LH_C(_lh_q_func_LH_C_0_3, _lh_q_func_LH_C_1_3) -> 
                          (match _lh_q_func_LH_C_1_3 with
                            | `LH_N -> 
                              (`Expr((`Func(_lh_q_func_MkString_0_0, ((map_lh__d7 unExpr_lh__d3) _lh_q_func_List_0_0)))))
                            | _ -> 
                              (failwith "lh_default_error"))
                        | _ -> 
                          (failwith "lh_default_error"))
                    | _ -> 
                      (failwith "lh_default_error"))
                | _ -> 
                  (failwith "lh_default_error"))
            | _ -> 
              (failwith "lh_default_error"))
        | _ -> 
          (failwith "lh_default_error"))
    | _ -> 
      (failwith "lh_default_error"));;
let rec orElseMap_lh__d1_d4 _lh_orElseMap_arg1_2 _lh_orElseMap_arg2_2 _lh_orElseMap_arg3_2 =
  ((orElse_lh__d6 (_lh_orElseMap_arg1_2 _lh_orElseMap_arg3_2)) (_lh_orElseMap_arg2_2 _lh_orElseMap_arg3_2));;
let rec seq2_lh__d6 _lh_seq2_arg1_2 _lh_seq2_arg2_2 _lh_seq2_arg3_2 _lh_funcomp_x_1_7 =
  (let rec g_3 = (fun xs_6 -> 
    (let rec _lh_matchIdent_3 = xs_6 in
      (match _lh_matchIdent_3 with
        | `LH_P2(_lh_seq2_LH_P2_0_2, _lh_seq2_LH_P2_1_2) -> 
          ((lift_lh__d6 ((cross_lh__d2 (_lh_seq2_arg1_2 _lh_seq2_LH_P2_0_2)) (fun x_2_3 -> 
            x_2_3))) (_lh_seq2_arg3_2 _lh_seq2_LH_P2_1_2))))) in
    ((fun _lh_funcomp_x_1_8 -> 
      (squash_lh__d1 ((lift_lh__d6 g_3) _lh_funcomp_x_1_8))) (_lh_seq2_arg2_2 _lh_funcomp_x_1_7)));;
let rec mk_cons_lh__d6 _lh_mk_cons_arg1_4 _lh_mk_cons_arg2_4 =
  (match _lh_mk_cons_arg2_4 with
    | `List(_lh_mk_cons_List_0_4) -> 
      (`List((`LH_C(_lh_mk_cons_arg1_4, _lh_mk_cons_List_0_4))))
    | _ -> 
      (failwith "lh_default_error"));;
let rec look_for_lh__d1_d7 _lh_look_for_arg1_1_3 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_5_6 -> 
    (x_5_6 = _lh_look_for_arg1_1_3))));;
let rec list_of_lh__d2 _lh_list_of_arg1_0 _lh_list_of_arg2_0 =
  (let rec p'_0 = (lazy (((seq2_lh__d6 mk_cons_lh__d6) _lh_list_of_arg1_0) ((orElseMap_lh__d1_d4 (((seq2Lzq_lh__d2 (fun x_3 y_0 -> 
    y_0)) (look_for_lh__d1_d7 _lh_list_of_arg2_0)) p'_0)) empty_lh__d6))) in
    (Lazy.force p'_0));;
let rec sp_lh__d7 _lh_sp_arg1_2 _lh_funcomp_x_6 =
  (_lh_sp_arg1_2 ((dropWhile_lh__d1 (fun x_1_4 -> 
    (x_1_4 = ' '))) _lh_funcomp_x_6));;
let rec p_ident_lh__d2 _lh_p_ident_arg1_2 =
  ((sp_lh__d7 (string_of_lh__d4 alphanum_lh__d2)) _lh_p_ident_arg1_2);;
let rec look_for_lh__d4 _lh_look_for_arg1_6 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_3_3 -> 
    (x_3_3 = _lh_look_for_arg1_6))));;
let rec look_for_lh__d1_d8 _lh_look_for_arg1_9 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_4_7 -> 
    (x_4_7 = _lh_look_for_arg1_9))));;
let rec look_for_lh__d5 _lh_look_for_arg1_7 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_4_2 -> 
    (x_4_2 = _lh_look_for_arg1_7))));;
let rec orElseMap_lh__d5 _lh_orElseMap_arg1_8 _lh_orElseMap_arg2_8 _lh_orElseMap_arg3_8 =
  ((orElse_lh__d6 (_lh_orElseMap_arg1_8 _lh_orElseMap_arg3_8)) (_lh_orElseMap_arg2_8 _lh_orElseMap_arg3_8));;
let rec seQ_lh__d5 _lh_seQ_arg1_1_1 _lh_seQ_arg2_1_1 =
  ((build_lh__d5 (mk_list_lh__d5 _lh_seQ_arg1_1_1)) (((foldr_lh__d6 (seq2_lh__d7 mk_cons_lh__d7)) empty_lh__d6) _lh_seQ_arg2_1_1));;
let rec foldr_lh__d3 f_5_9 i_5_3 ls_5 =
  ((ls_5 f_5_9) i_5_3);;
let rec orElse_lh__d2 _lh_orElse_arg1_3 _lh_orElse_arg2_3 =
  (match _lh_orElse_arg1_3 with
    | `Nothing -> 
      _lh_orElse_arg2_3
    | _ -> 
      _lh_orElse_arg1_3);;
let rec orElseMap_lh__d1 _lh_orElseMap_arg1_1_1 _lh_orElseMap_arg2_1_1 _lh_orElseMap_arg3_1_1 =
  ((orElse_lh__d2 (_lh_orElseMap_arg1_1_1 _lh_orElseMap_arg3_1_1)) (_lh_orElseMap_arg2_1_1 _lh_orElseMap_arg3_1_1));;
let rec orElse_lh__d5 _lh_orElse_arg1_2 _lh_orElse_arg2_2 =
  (match _lh_orElse_arg1_2 with
    | `Nothing -> 
      _lh_orElse_arg2_2
    | _ -> 
      _lh_orElse_arg1_2);;
let rec orElseMap_lh__d4 _lh_orElseMap_arg1_1 _lh_orElseMap_arg2_1 _lh_orElseMap_arg3_1 =
  ((orElse_lh__d5 (_lh_orElseMap_arg1_1 _lh_orElseMap_arg3_1)) (_lh_orElseMap_arg2_1 _lh_orElseMap_arg3_1));;
let rec q_func_lh__d1 _lh_q_func_arg1_1 =
  (match _lh_q_func_arg1_1 with
    | `LH_C(_lh_q_func_LH_C_0_4, _lh_q_func_LH_C_1_4) -> 
      (match _lh_q_func_LH_C_0_4 with
        | `MkString(_lh_q_func_MkString_0_1) -> 
          (match _lh_q_func_LH_C_1_4 with
            | `LH_C(_lh_q_func_LH_C_0_5, _lh_q_func_LH_C_1_5) -> 
              (match _lh_q_func_LH_C_1_5 with
                | `LH_C(_lh_q_func_LH_C_0_6, _lh_q_func_LH_C_1_6) -> 
                  (match _lh_q_func_LH_C_0_6 with
                    | `List(_lh_q_func_List_0_1) -> 
                      (match _lh_q_func_LH_C_1_6 with
                        | `LH_C(_lh_q_func_LH_C_0_7, _lh_q_func_LH_C_1_7) -> 
                          (match _lh_q_func_LH_C_1_7 with
                            | `LH_N -> 
                              (`Expr((`Func(_lh_q_func_MkString_0_1, ((map_lh__d7 unExpr_lh__d3) _lh_q_func_List_0_1)))))
                            | _ -> 
                              (failwith "lh_default_error"))
                        | _ -> 
                          (failwith "lh_default_error"))
                    | _ -> 
                      (failwith "lh_default_error"))
                | _ -> 
                  (failwith "lh_default_error"))
            | _ -> 
              (failwith "lh_default_error"))
        | _ -> 
          (failwith "lh_default_error"))
    | _ -> 
      (failwith "lh_default_error"));;
let rec seQ_lh__d1_d2 _lh_seQ_arg1_4 _lh_seQ_arg2_4 =
  ((build_lh__d5 (mk_list_lh__d5 _lh_seQ_arg1_4)) (((foldr_lh__d6 (seq2_lh__d7 mk_cons_lh__d7)) empty_lh__d6) _lh_seQ_arg2_4));;
let rec p_name_lh__d1 _lh_p_name_arg1_1 =
  (((build_lh__d5 q_name_lh__d1) p_ident_lh__d6) _lh_p_name_arg1_1);;
let rec look_for_lh__d1_d5 _lh_look_for_arg1_1_4 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_5_7 -> 
    (x_5_7 = _lh_look_for_arg1_1_4))));;
let rec atIndex_lh__d1 n_1 ls_2_0 =
  (if (n_1 < 0) then
    (failwith "lh_default_error")
  else
    (match ls_2_0 with
      | `LH_C(h_1_0_0, t_1_0_3) -> 
        (if (n_1 = 0) then
          h_1_0_0
        else
          ((atIndex_lh__d1 (n_1 - 1)) t_1_0_3))
      | `LH_N -> 
        (failwith "lh_default_error")));;
let rec orElseMap_lh__d1_d3 _lh_orElseMap_arg1_1_0 _lh_orElseMap_arg2_1_0 _lh_orElseMap_arg3_1_0 =
  ((orElse_lh__d6 (_lh_orElseMap_arg1_1_0 _lh_orElseMap_arg3_1_0)) (_lh_orElseMap_arg2_1_0 _lh_orElseMap_arg3_1_0));;
let rec look_for_lh__d1_d6 _lh_look_for_arg1_1_6 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_5_9 -> 
    (x_5_9 = _lh_look_for_arg1_1_6))));;
let rec sp_lh__d4 _lh_sp_arg1_5 _lh_funcomp_x_1_1 =
  (_lh_sp_arg1_5 ((dropWhile_lh__d1 (fun x_1_8 -> 
    (x_1_8 = ' '))) _lh_funcomp_x_1_1));;
let rec pchar_lh__d2 _lh_pchar_arg1_2 _lh_pchar_arg2_2 =
  (match _lh_pchar_arg2_2 with
    | `LH_N -> 
      croak_lh__d1
    | `LH_C(_lh_pchar_LH_C_0_2, _lh_pchar_LH_C_1_2) -> 
      (if (_lh_pchar_arg1_2 _lh_pchar_LH_C_0_2) then
        (succeed_lh__d1 (`LH_P2((`MkString((`LH_C(_lh_pchar_LH_C_0_2, (`LH_N))))), _lh_pchar_LH_C_1_2)))
      else
        croak_lh__d1));;
let rec look_for_lh__d2 _lh_look_for_arg1_3 =
  (sp_lh__d4 (pchar_lh__d2 (fun x_2_4 -> 
    (x_2_4 = _lh_look_for_arg1_3))));;
let rec orElseMap_lh__d1_d2 _lh_orElseMap_arg1_4 _lh_orElseMap_arg2_4 _lh_orElseMap_arg3_4 =
  ((orElse_lh__d6 (_lh_orElseMap_arg1_4 _lh_orElseMap_arg3_4)) (_lh_orElseMap_arg2_4 _lh_orElseMap_arg3_4));;
let rec look_for_lh__d1_d4 _lh_look_for_arg1_1 =
  (sp_lh__d8 (pchar_lh__d4 (fun x_1_1 -> 
    (x_1_1 = _lh_look_for_arg1_1))));;
let rec mk_cons_lh__d5 _lh_mk_cons_arg1_5 _lh_mk_cons_arg2_5 =
  (match _lh_mk_cons_arg2_5 with
    | `List(_lh_mk_cons_List_0_5) -> 
      (`List((`LH_C(_lh_mk_cons_arg1_5, _lh_mk_cons_List_0_5))))
    | _ -> 
      (failwith "lh_default_error"));;
let rec empty_lh__d5 _lh_empty_arg1_4 =
  (succeed_lh__d1 (`LH_P2((`List((`LH_N))), _lh_empty_arg1_4)));;
let rec seq2_lh__d5 _lh_seq2_arg1_6 _lh_seq2_arg2_6 _lh_seq2_arg3_6 _lh_funcomp_x_3_5 =
  (let rec g_7 = (fun xs_1_1 -> 
    (let rec _lh_matchIdent_1_7 = xs_1_1 in
      (match _lh_matchIdent_1_7 with
        | `LH_P2(_lh_seq2_LH_P2_0_6, _lh_seq2_LH_P2_1_6) -> 
          ((lift_lh__d6 ((cross_lh__d2 (_lh_seq2_arg1_6 _lh_seq2_LH_P2_0_6)) (fun x_6_9 -> 
            x_6_9))) (_lh_seq2_arg3_6 _lh_seq2_LH_P2_1_6))))) in
    ((fun _lh_funcomp_x_3_6 -> 
      (squash_lh__d1 ((lift_lh__d6 g_7) _lh_funcomp_x_3_6))) (_lh_seq2_arg2_6 _lh_funcomp_x_3_5)));;
let rec seq2Lzq_lh__d1 _lh_seq2Lzq_arg1_1 _lh_seq2Lzq_arg2_1 _lh_seq2Lzq_arg3_1 _lh_funcomp_x_3_7 =
  (let rec g_8 = (fun xs_1_2 -> 
    (let rec _lh_matchIdent_1_8 = xs_1_2 in
      (match _lh_matchIdent_1_8 with
        | `LH_P2(_lh_seq2Lzq_LH_P2_0_1, _lh_seq2Lzq_LH_P2_1_1) -> 
          ((lift_lh__d6 ((cross_lh__d2 (_lh_seq2Lzq_arg1_1 _lh_seq2Lzq_LH_P2_0_1)) (fun x_7_0 -> 
            x_7_0))) ((Lazy.force _lh_seq2Lzq_arg3_1) _lh_seq2Lzq_LH_P2_1_1))))) in
    ((fun _lh_funcomp_x_3_8 -> 
      (squash_lh__d1 ((lift_lh__d6 g_8) _lh_funcomp_x_3_8))) (_lh_seq2Lzq_arg2_1 _lh_funcomp_x_3_7)));;
let rec list_of_lh__d1 _lh_list_of_arg1_3 _lh_list_of_arg2_3 =
  (let rec p'_3 = (lazy (((seq2_lh__d5 mk_cons_lh__d5) _lh_list_of_arg1_3) ((orElseMap_lh__d1_d2 (((seq2Lzq_lh__d1 (fun x_4_5 y_5 -> 
    y_5)) (look_for_lh__d1_d4 _lh_list_of_arg2_3)) p'_3)) empty_lh__d5))) in
    (Lazy.force p'_3));;
let rec sp_lh__d6 _lh_sp_arg1_4 _lh_funcomp_x_1_0 =
  (_lh_sp_arg1_4 ((dropWhile_lh__d1 (fun x_1_7 -> 
    (x_1_7 = ' '))) _lh_funcomp_x_1_0));;
let rec pchar_lh__d3 _lh_pchar_arg1_0 _lh_pchar_arg2_0 =
  (match _lh_pchar_arg2_0 with
    | `LH_N -> 
      croak_lh__d1
    | `LH_C(_lh_pchar_LH_C_0_0, _lh_pchar_LH_C_1_0) -> 
      (if (_lh_pchar_arg1_0 _lh_pchar_LH_C_0_0) then
        (succeed_lh__d1 (`LH_P2((`MkString((`LH_C(_lh_pchar_LH_C_0_0, (`LH_N))))), _lh_pchar_LH_C_1_0)))
      else
        croak_lh__d1));;
let rec look_for_lh__d3 _lh_look_for_arg1_0 =
  (sp_lh__d6 (pchar_lh__d3 (fun x_8 -> 
    (x_8 = _lh_look_for_arg1_0))));;
let rec alphanum_lh__d1 _lh_alphanum_arg1_1 =
  (((((int_of_char _lh_alphanum_arg1_1) >= (int_of_char 'A')) && ((int_of_char _lh_alphanum_arg1_1) <= (int_of_char 'Z'))) || (((int_of_char _lh_alphanum_arg1_1) >= (int_of_char 'a')) && ((int_of_char _lh_alphanum_arg1_1) <= (int_of_char 'z')))) || (((int_of_char _lh_alphanum_arg1_1) >= (int_of_char '0')) && ((int_of_char _lh_alphanum_arg1_1) <= (int_of_char '9'))));;
let rec sp_lh__d5 _lh_sp_arg1_7 _lh_funcomp_x_3_4 =
  (_lh_sp_arg1_7 ((dropWhile_lh__d1 (fun x_6_7 -> 
    (x_6_7 = ' '))) _lh_funcomp_x_3_4));;
let rec string_of_lh__d3 _lh_string_of_arg1_0 _lh_string_of_arg2_0 =
  (let rec chars_0 = ((takeWhile_lh__d1 _lh_string_of_arg1_0) _lh_string_of_arg2_0) in
    (let rec s'_0 = ((dropWhile_lh__d1 _lh_string_of_arg1_0) _lh_string_of_arg2_0) in
      ((maybeX_lh__d1 (chars_0 <> (`LH_N))) (`LH_P2((`MkString(chars_0)), s'_0)))));;
let rec p_ident_lh__d1 _lh_p_ident_arg1_5 =
  ((sp_lh__d5 (string_of_lh__d3 alphanum_lh__d1)) _lh_p_ident_arg1_5);;
let rec mk_cons_lh__d4 _lh_mk_cons_arg1_1 _lh_mk_cons_arg2_1 =
  (match _lh_mk_cons_arg2_1 with
    | `List(_lh_mk_cons_List_0_1) -> 
      (`List((`LH_C(_lh_mk_cons_arg1_1, _lh_mk_cons_List_0_1))))
    | _ -> 
      (failwith "lh_default_error"));;
let rec seq2_lh__d4 _lh_seq2_arg1_1 _lh_seq2_arg2_1 _lh_seq2_arg3_1 _lh_funcomp_x_7 =
  (let rec g_1 = (fun xs_3 -> 
    (let rec _lh_matchIdent_1 = xs_3 in
      (match _lh_matchIdent_1 with
        | `LH_P2(_lh_seq2_LH_P2_0_1, _lh_seq2_LH_P2_1_1) -> 
          ((lift_lh__d6 ((cross_lh__d2 (_lh_seq2_arg1_1 _lh_seq2_LH_P2_0_1)) (fun x_1_5 -> 
            x_1_5))) (_lh_seq2_arg3_1 _lh_seq2_LH_P2_1_1))))) in
    ((fun _lh_funcomp_x_8 -> 
      (squash_lh__d1 ((lift_lh__d6 g_1) _lh_funcomp_x_8))) (_lh_seq2_arg2_1 _lh_funcomp_x_7)));;
let rec mk_list_lh__d4 _lh_mk_list_arg1_2 _lh_mk_list_arg2_2 =
  (match _lh_mk_list_arg2_2 with
    | `List(_lh_mk_list_List_0_2) -> 
      (_lh_mk_list_arg1_2 _lh_mk_list_List_0_2)
    | _ -> 
      (failwith "lh_default_error"));;
let rec build_lh__d4 _lh_build_arg1_2 _lh_build_arg2_2 _lh_funcomp_x_1_9 =
  ((lift_lh__d6 ((cross_lh__d2 _lh_build_arg1_2) (fun x_2_7 -> 
    x_2_7))) (_lh_build_arg2_2 _lh_funcomp_x_1_9));;
let rec empty_lh__d4 _lh_empty_arg1_5 =
  (succeed_lh__d1 (`LH_P2((`List((`LH_N))), _lh_empty_arg1_5)));;
let rec seQ_lh__d4 _lh_seQ_arg1_8 _lh_seQ_arg2_8 =
  ((build_lh__d4 (mk_list_lh__d4 _lh_seQ_arg1_8)) (((foldr_lh__d6 (seq2_lh__d4 mk_cons_lh__d4)) empty_lh__d4) _lh_seQ_arg2_8));;
let rec q_op_lh__d1 _lh_q_op_arg1_3 =
  (match _lh_q_op_arg1_3 with
    | `LH_C(_lh_q_op_LH_C_0_9, _lh_q_op_LH_C_1_9) -> 
      (match _lh_q_op_LH_C_0_9 with
        | `Expr(_lh_q_op_Expr_0_6) -> 
          (match _lh_q_op_LH_C_1_9 with
            | `LH_C(_lh_q_op_LH_C_0_1_0, _lh_q_op_LH_C_1_1_0) -> 
              (match _lh_q_op_LH_C_0_1_0 with
                | `MkString(_lh_q_op_MkString_0_3) -> 
                  (match _lh_q_op_LH_C_1_1_0 with
                    | `LH_C(_lh_q_op_LH_C_0_1_1, _lh_q_op_LH_C_1_1_1) -> 
                      (match _lh_q_op_LH_C_0_1_1 with
                        | `Expr(_lh_q_op_Expr_0_7) -> 
                          (match _lh_q_op_LH_C_1_1_1 with
                            | `LH_N -> 
                              (`Expr((`Func(_lh_q_op_MkString_0_3, (`LH_C(_lh_q_op_Expr_0_6, (`LH_C(_lh_q_op_Expr_0_7, (`LH_N)))))))))
                            | _ -> 
                              (failwith "lh_default_error"))
                        | _ -> 
                          (failwith "lh_default_error"))
                    | _ -> 
                      (failwith "lh_default_error"))
                | _ -> 
                  (failwith "lh_default_error"))
            | _ -> 
              (failwith "lh_default_error"))
        | _ -> 
          (failwith "lh_default_error"))
    | _ -> 
      (failwith "lh_default_error"));;
let rec sp_lh__d1 _lh_sp_arg1_0 _lh_funcomp_x_0 =
  (_lh_sp_arg1_0 ((dropWhile_lh__d1 (fun x_0 -> 
    (x_0 = ' '))) _lh_funcomp_x_0));;
let rec opsym_lh__d1 _lh_opsym_arg1_1 =
  ((fun x_2_0 -> 
    ((inList_lh__d4 x_2_0) (`LH_C('*', (`LH_C('+', (`LH_C('%', (`LH_C('@', (`LH_C('-', (`LH_C('/', (`LH_C('?', (`LH_C(':', (`LH_N))))))))))))))))))) _lh_opsym_arg1_1);;
let rec string_of_lh__d1 _lh_string_of_arg1_3 _lh_string_of_arg2_3 =
  (let rec chars_3 = ((takeWhile_lh__d1 _lh_string_of_arg1_3) _lh_string_of_arg2_3) in
    (let rec s'_3 = ((dropWhile_lh__d1 _lh_string_of_arg1_3) _lh_string_of_arg2_3) in
      ((maybeX_lh__d1 (chars_3 <> (`LH_N))) (`LH_P2((`MkString(chars_3)), s'_3)))));;
let rec p_op_lh__d1 _lh_p_op_arg1_3 =
  ((sp_lh__d1 (string_of_lh__d1 opsym_lh__d1)) _lh_p_op_arg1_3);;
let rec mk_cons_lh__d1 _lh_mk_cons_arg1_0 _lh_mk_cons_arg2_0 =
  (match _lh_mk_cons_arg2_0 with
    | `List(_lh_mk_cons_List_0_0) -> 
      (`List((`LH_C(_lh_mk_cons_arg1_0, _lh_mk_cons_List_0_0))))
    | _ -> 
      (failwith "lh_default_error"));;
let rec empty_lh__d1 _lh_empty_arg1_0 =
  (succeed_lh__d1 (`LH_P2((`List((`LH_N))), _lh_empty_arg1_0)));;
let rec mk_list_lh__d1 _lh_mk_list_arg1_3 _lh_mk_list_arg2_3 =
  (match _lh_mk_list_arg2_3 with
    | `List(_lh_mk_list_List_0_3) -> 
      (_lh_mk_list_arg1_3 _lh_mk_list_List_0_3)
    | _ -> 
      (failwith "lh_default_error"));;
let rec cross_lh__d1 _lh_cross_arg1_1 _lh_cross_arg2_1 _lh_cross_arg3_1 =
  (match _lh_cross_arg3_1 with
    | `LH_P2(_lh_cross_LH_P2_0_1, _lh_cross_LH_P2_1_1) -> 
      (`LH_P2((_lh_cross_arg1_1 _lh_cross_LH_P2_0_1), (_lh_cross_arg2_1 _lh_cross_LH_P2_1_1))));;
let rec lift_lh__d4 _lh_lift_arg1_1 _lh_lift_arg2_1 =
  (match _lh_lift_arg2_1 with
    | `Nothing -> 
      (`Nothing)
    | `Just(_lh_lift_Just_0_1) -> 
      (`Just((_lh_lift_arg1_1 _lh_lift_Just_0_1))));;
let rec lift_lh__d5 _lh_lift_arg1_2 _lh_lift_arg2_2 =
  (match _lh_lift_arg2_2 with
    | `Nothing -> 
      (`Nothing)
    | `Just(_lh_lift_Just_0_2) -> 
      (`Just((_lh_lift_arg1_2 _lh_lift_Just_0_2))));;
let rec seq2_lh__d1 _lh_seq2_arg1_3 _lh_seq2_arg2_3 _lh_seq2_arg3_3 _lh_funcomp_x_2_5 =
  (let rec g_4 = (fun xs_7 -> 
    (let rec _lh_matchIdent_5 = xs_7 in
      (match _lh_matchIdent_5 with
        | `LH_P2(_lh_seq2_LH_P2_0_3, _lh_seq2_LH_P2_1_3) -> 
          ((lift_lh__d5 ((cross_lh__d1 (_lh_seq2_arg1_3 _lh_seq2_LH_P2_0_3)) (fun x_4_1 -> 
            x_4_1))) (_lh_seq2_arg3_3 _lh_seq2_LH_P2_1_3))))) in
    ((fun _lh_funcomp_x_2_6 -> 
      (squash_lh__d1 ((lift_lh__d4 g_4) _lh_funcomp_x_2_6))) (_lh_seq2_arg2_3 _lh_funcomp_x_2_5)));;
let rec build_lh__d1 _lh_build_arg1_3 _lh_build_arg2_3 _lh_funcomp_x_2_3 =
  ((lift_lh__d6 ((cross_lh__d2 _lh_build_arg1_3) (fun x_2_8 -> 
    x_2_8))) (_lh_build_arg2_3 _lh_funcomp_x_2_3));;
let rec seQ_lh__d1 _lh_seQ_arg1_1_0 _lh_seQ_arg2_1_0 =
  ((build_lh__d1 (mk_list_lh__d1 _lh_seQ_arg1_1_0)) (((foldr_lh__d3 (seq2_lh__d1 mk_cons_lh__d1)) empty_lh__d1) _lh_seQ_arg2_1_0));;
let rec p_expr_lh__d1 _lh_p_expr_arg1_2 =
  (((orElseMap_lh__d1 ((seQ_lh__d1 q_op_lh__d1) (let rec t_3_4 = (let rec t_3_5 = (let rec t_3_6 = (fun f_4_4 i_3_8 -> 
    i_3_8) in
    (let rec h_3_2 = p_term_lh__d2 in
      (fun f_4_5 i_3_9 -> 
        ((f_4_5 h_3_2) (((foldr_lh__d3 f_4_5) i_3_9) t_3_6))))) in
    (let rec h_3_3 = p_op_lh__d1 in
      (fun f_4_6 i_4_0 -> 
        ((f_4_6 h_3_3) (((foldr_lh__d3 f_4_6) i_4_0) t_3_5))))) in
    (let rec h_3_4 = p_term_lh__d3 in
      (fun f_4_7 i_4_1 -> 
        ((f_4_7 h_3_4) (((foldr_lh__d3 f_4_7) i_4_1) t_3_4))))))) p_term_lh__d1) _lh_p_expr_arg1_2)
and
p_prim_lh__d1 _lh_p_prim_arg1_1 =
  (((orElseMap_lh__d1_d3 p_name_lh__d1) ((seQ_lh__d1_d2 (fun x_1_2 -> 
    ((atIndex_lh__d1 1) x_1_2))) (let rec t_1_8 = (let rec t_1_9 = (let rec t_2_0 = (fun f_2_3 i_2_1 -> 
    i_2_1) in
    (let rec h_1_6 = (look_for_lh__d1_d6 ')') in
      (fun f_2_4 i_2_2 -> 
        ((f_2_4 h_1_6) (((foldr_lh__d6 f_2_4) i_2_2) t_2_0))))) in
    (let rec h_1_7 = p_expr_lh__d1 in
      (fun f_2_5 i_2_3 -> 
        ((f_2_5 h_1_7) (((foldr_lh__d6 f_2_5) i_2_3) t_1_9))))) in
    (let rec h_1_8 = (look_for_lh__d1_d5 '(') in
      (fun f_2_6 i_2_4 -> 
        ((f_2_6 h_1_8) (((foldr_lh__d6 f_2_6) i_2_4) t_1_8))))))) _lh_p_prim_arg1_1)
and
p_prim_lh__d2 _lh_p_prim_arg1_2 =
  (((orElseMap_lh__d1_d5 p_name_lh__d2) ((seQ_lh__d1_d3 (fun x_3_5 -> 
    ((atIndex_lh__d2 1) x_3_5))) (let rec t_5_1 = (let rec t_5_2 = (let rec t_5_3 = (fun f_6_5 i_5_9 -> 
    i_5_9) in
    (let rec h_4_8 = (look_for_lh__d1_d9 ')') in
      (fun f_6_6 i_6_0 -> 
        ((f_6_6 h_4_8) (((foldr_lh__d6 f_6_6) i_6_0) t_5_3))))) in
    (let rec h_4_9 = p_expr_lh__d1 in
      (fun f_6_7 i_6_1 -> 
        ((f_6_7 h_4_9) (((foldr_lh__d6 f_6_7) i_6_1) t_5_2))))) in
    (let rec h_5_0 = (look_for_lh__d1_d8 '(') in
      (fun f_6_8 i_6_2 -> 
        ((f_6_8 h_5_0) (((foldr_lh__d6 f_6_8) i_6_2) t_5_1))))))) _lh_p_prim_arg1_2)
and
p_prim_lh__d3 _lh_p_prim_arg1_3 =
  (((orElseMap_lh__d1_d5 p_name_lh__d2) ((seQ_lh__d1_d3 (fun x_3_7 -> 
    ((atIndex_lh__d2 1) x_3_7))) (let rec t_6_0 = (let rec t_6_1 = (let rec t_6_2 = (fun f_7_5 i_6_8 -> 
    i_6_8) in
    (let rec h_5_7 = (look_for_lh__d1_d9 ')') in
      (fun f_7_6 i_6_9 -> 
        ((f_7_6 h_5_7) (((foldr_lh__d6 f_7_6) i_6_9) t_6_2))))) in
    (let rec h_5_8 = p_expr_lh__d1 in
      (fun f_7_7 i_7_0 -> 
        ((f_7_7 h_5_8) (((foldr_lh__d6 f_7_7) i_7_0) t_6_1))))) in
    (let rec h_5_9 = (look_for_lh__d1_d9 '(') in
      (fun f_7_8 i_7_1 -> 
        ((f_7_8 h_5_9) (((foldr_lh__d6 f_7_8) i_7_1) t_6_0))))))) _lh_p_prim_arg1_3)
and
p_term_lh__d1 _lh_p_term_arg1_1_1 =
  (((orElseMap_lh__d4 ((seQ_lh__d4 q_func_lh__d1) (let rec t_9_7 = (let rec t_9_8 = (let rec t_9_9 = (let rec t_1_0_0 = (fun f_1_2_2 i_1_0_9 -> 
    i_1_0_9) in
    (let rec h_9_4 = (look_for_lh__d2 ')') in
      (fun f_1_2_3 i_1_1_0 -> 
        ((f_1_2_3 h_9_4) (((foldr_lh__d6 f_1_2_3) i_1_1_0) t_1_0_0))))) in
    (let rec h_9_5 = ((list_of_lh__d1 p_expr_lh__d1) ',') in
      (fun f_1_2_4 i_1_1_1 -> 
        ((f_1_2_4 h_9_5) (((foldr_lh__d6 f_1_2_4) i_1_1_1) t_9_9))))) in
    (let rec h_9_6 = (look_for_lh__d3 '(') in
      (fun f_1_2_5 i_1_1_2 -> 
        ((f_1_2_5 h_9_6) (((foldr_lh__d6 f_1_2_5) i_1_1_2) t_9_8))))) in
    (let rec h_9_7 = p_ident_lh__d1 in
      (fun f_1_2_6 i_1_1_3 -> 
        ((f_1_2_6 h_9_7) (((foldr_lh__d6 f_1_2_6) i_1_1_3) t_9_7))))))) p_prim_lh__d1) _lh_p_term_arg1_1_1)
and
p_term_lh__d2 _lh_p_term_arg1_1 =
  (((orElseMap_lh__d5 ((seQ_lh__d5 q_func_lh__d2) (let rec t_4 = (let rec t_5 = (let rec t_6 = (let rec t_7 = (fun f_5 i_5 -> 
    i_5) in
    (let rec h_4 = (look_for_lh__d4 ')') in
      (fun f_6 i_6 -> 
        ((f_6 h_4) (((foldr_lh__d6 f_6) i_6) t_7))))) in
    (let rec h_5 = ((list_of_lh__d2 p_expr_lh__d1) ',') in
      (fun f_7 i_7 -> 
        ((f_7 h_5) (((foldr_lh__d6 f_7) i_7) t_6))))) in
    (let rec h_6 = (look_for_lh__d5 '(') in
      (fun f_8 i_8 -> 
        ((f_8 h_6) (((foldr_lh__d6 f_8) i_8) t_5))))) in
    (let rec h_7 = p_ident_lh__d2 in
      (fun f_9 i_9 -> 
        ((f_9 h_7) (((foldr_lh__d6 f_9) i_9) t_4))))))) p_prim_lh__d2) _lh_p_term_arg1_1)
and
p_term_lh__d3 _lh_p_term_arg1_0 =
  (((orElseMap_lh__d6 ((seQ_lh__d6 q_func_lh__d3) (let rec t_0 = (let rec t_1 = (let rec t_2 = (let rec t_3 = (fun f_0 i_0 -> 
    i_0) in
    (let rec h_0 = (look_for_lh__d6 ')') in
      (fun f_1 i_1 -> 
        ((f_1 h_0) (((foldr_lh__d6 f_1) i_1) t_3))))) in
    (let rec h_1 = ((list_of_lh__d3 p_expr_lh__d1) ',') in
      (fun f_2 i_2 -> 
        ((f_2 h_1) (((foldr_lh__d6 f_2) i_2) t_2))))) in
    (let rec h_2 = (look_for_lh__d7 '(') in
      (fun f_3 i_3 -> 
        ((f_3 h_2) (((foldr_lh__d6 f_3) i_3) t_1))))) in
    (let rec h_3 = p_ident_lh__d3 in
      (fun f_4 i_4 -> 
        ((f_4 h_3) (((foldr_lh__d6 f_4) i_4) t_0))))))) p_prim_lh__d3) _lh_p_term_arg1_0);;
let rec fst_lh__d1 _lh_fst_arg1_1 =
  (match _lh_fst_arg1_1 with
    | `LH_P2(_lh_fst_LH_P2_0_1, _lh_fst_LH_P2_1_1) -> 
      _lh_fst_LH_P2_0_1);;
let rec parse_lh__d1 _lh_parse_arg1_1 =
  ((fun _lh_funcomp_x_2_0 -> 
    ((fun _lh_funcomp_x_2_1 -> 
      ((fun _lh_funcomp_x_2_2 -> 
        (unExpr_lh__d1 (fst_lh__d1 _lh_funcomp_x_2_2))) (the_lh__d1 _lh_funcomp_x_2_1))) (p_expr_lh__d1 _lh_funcomp_x_2_0))) _lh_parse_arg1_1);;
let rec all_lh__d1 _lh_all_arg1_5 _lh_all_arg2_3 =
  (_lh_all_arg2_3 _lh_all_arg1_5);;
let rec copy_lh__d1 _lh_copy_arg1_0 _lh_copy_arg2_0 _lh_floatOutId_0_0 =
  (if (_lh_copy_arg1_0 <= 0) then
    true
  else
    (let rec _lh_all_LH_C_1_1 = ((copy_lh__d1 (_lh_copy_arg1_0 - 1)) _lh_copy_arg2_0) in
      (let rec _lh_all_LH_C_0_1 = _lh_copy_arg2_0 in
        (if (_lh_floatOutId_0_0 _lh_all_LH_C_0_1) then
          ((all_lh__d1 _lh_floatOutId_0_0) _lh_all_LH_C_1_1)
        else
          false))));;
let rec lex_ext_lh__d1 _lh_lex_ext_arg1_0 _lh_lex_ext_arg2_0 =
  (_lh_lex_ext_arg2_0 _lh_lex_ext_arg1_0);;
let rec map_lh__d1 f_9_2 ls_1_2 =
  (ls_1_2 f_9_2);;
let rec quant_lh__d5 _lh_quant_arg1_0 _lh_quant_arg2_0 _lh_quant_arg3_0 _lh_quant_arg4_0 =
  (let rec p_0 = (fun x_5 -> 
    ((_lh_quant_arg2_0 x_5) _lh_quant_arg3_0)) in
    ((_lh_quant_arg1_0 p_0) _lh_quant_arg4_0));;
let rec inList_lh__d3 _lh_inList_arg1_4 _lh_inList_arg2_1 =
  (_lh_inList_arg2_1 _lh_inList_arg1_4);;
let rec geRewrite_lh__d3 _lh_geRewrite_arg1_0 _lh_geRewrite_arg2_0 _lh_geRewrite_arg3_0 =
  ((inList_lh__d3 (_lh_geRewrite_arg1_0 (`LH_P2(_lh_geRewrite_arg2_0, _lh_geRewrite_arg3_0)))) (let rec _lh_inList_LH_C_1_0 = (let rec _lh_inList_LH_C_1_1 = (fun _lh_inList_arg1_0 -> 
    false) in
    (let rec _lh_inList_LH_C_0_0 = (`Equal) in
      (fun _lh_inList_arg1_1 -> 
        (if (_lh_inList_arg1_1 = _lh_inList_LH_C_0_0) then
          true
        else
          ((inList_lh__d3 _lh_inList_arg1_1) _lh_inList_LH_C_1_1))))) in
    (let rec _lh_inList_LH_C_0_1 = (`Greater) in
      (fun _lh_inList_arg1_2 -> 
        (if (_lh_inList_arg1_2 = _lh_inList_LH_C_0_1) then
          true
        else
          ((inList_lh__d3 _lh_inList_arg1_2) _lh_inList_LH_C_1_0))))));;
let rec any_lh__d3 _lh_any_arg1_0 _lh_any_arg2_0 =
  (match _lh_any_arg2_0 with
    | `LH_N -> 
      false
    | `LH_C(_lh_any_LH_C_0_0, _lh_any_LH_C_1_0) -> 
      (if (_lh_any_arg1_0 _lh_any_LH_C_0_0) then
        true
      else
        ((any_lh__d3 _lh_any_arg1_0) _lh_any_LH_C_1_0)));;
let rec geRewrite_lh__d4 _lh_geRewrite_arg1_1 _lh_geRewrite_arg2_1 _lh_geRewrite_arg3_1 =
  ((inList_lh__d4 (_lh_geRewrite_arg1_1 (`LH_P2(_lh_geRewrite_arg2_1, _lh_geRewrite_arg3_1)))) (`LH_C((`Greater), (`LH_C((`Equal), (`LH_N))))));;
let rec ltRewrite_lh__d2 _lh_ltRewrite_arg1_0 _lh_ltRewrite_arg2_0 _lh_ltRewrite_arg3_0 =
  ((_lh_ltRewrite_arg1_0 (`LH_P2(_lh_ltRewrite_arg2_0, _lh_ltRewrite_arg3_0))) = (`Less));;
let rec any_lh__d2 _lh_any_arg1_1 _lh_any_arg2_1 =
  (match _lh_any_arg2_1 with
    | `LH_N -> 
      false
    | `LH_C(_lh_any_LH_C_0_1, _lh_any_LH_C_1_1) -> 
      (if (_lh_any_arg1_1 _lh_any_LH_C_0_1) then
        true
      else
        ((any_lh__d2 _lh_any_arg1_1) _lh_any_LH_C_1_1)));;
let rec quant_lh__d7 _lh_quant_arg1_2 _lh_quant_arg2_2 _lh_quant_arg3_2 _lh_quant_arg4_2 =
  (let rec p_2 = (fun x_4_0 -> 
    ((_lh_quant_arg2_2 x_4_0) _lh_quant_arg3_2)) in
    ((_lh_quant_arg1_2 p_2) _lh_quant_arg4_2));;
let rec ltRewrite_lh__d4 _lh_ltRewrite_arg1_2 _lh_ltRewrite_arg2_2 _lh_ltRewrite_arg3_2 =
  ((_lh_ltRewrite_arg1_2 (`LH_P2(_lh_ltRewrite_arg2_2, _lh_ltRewrite_arg3_2))) = (`Less));;
let rec all_lh__d2 _lh_all_arg1_4 _lh_all_arg2_2 =
  (match _lh_all_arg2_2 with
    | `LH_N -> 
      true
    | `LH_C(_lh_all_LH_C_0_3, _lh_all_LH_C_1_3) -> 
      (if (_lh_all_arg1_4 _lh_all_LH_C_0_3) then
        ((all_lh__d2 _lh_all_arg1_4) _lh_all_LH_C_1_3)
      else
        false));;
let rec any_lh__d1 _lh_any_arg1_3 _lh_any_arg2_3 =
  (match _lh_any_arg2_3 with
    | `LH_N -> 
      false
    | `LH_C(_lh_any_LH_C_0_3, _lh_any_LH_C_1_3) -> 
      (if (_lh_any_arg1_3 _lh_any_LH_C_0_3) then
        true
      else
        ((any_lh__d1 _lh_any_arg1_3) _lh_any_LH_C_1_3)));;
let rec quant_lh__d4 _lh_quant_arg1_7 _lh_quant_arg2_7 _lh_quant_arg3_7 _lh_quant_arg4_7 =
  (let rec p_7 = (fun x_6_8 -> 
    ((_lh_quant_arg2_7 x_6_8) _lh_quant_arg3_7)) in
    ((_lh_quant_arg1_7 p_7) _lh_quant_arg4_7));;
let rec or_lh__d2 _lh_or_arg1_1 =
  (_lh_or_arg1_1 99);;
let rec map_lh__d4 f_1_1_2 ls_1_7 _lh_floatOutId_0_8 =
  (match ls_1_7 with
    | `LH_C(h_8_6, t_8_9) -> 
      (let rec _lh_or_LH_C_1_1 = ((map_lh__d4 f_1_1_2) t_8_9) in
        (let rec _lh_or_LH_C_0_1 = (f_1_1_2 h_8_6) in
          (if _lh_or_LH_C_0_1 then
            true
          else
            (or_lh__d2 _lh_or_LH_C_1_1))))
    | `LH_N -> 
      false);;
let rec expr_fold_lh__d2 _lh_expr_fold_arg1_2 _lh_expr_fold_arg2_2 _lh_expr_fold_arg3_2 =
  (match _lh_expr_fold_arg3_2 with
    | `Var(_lh_expr_fold_Var_0_2) -> 
      (_lh_expr_fold_arg2_2 _lh_expr_fold_Var_0_2)
    | `Func(_lh_expr_fold_Func_0_2, _lh_expr_fold_Func_1_2) -> 
      ((_lh_expr_fold_arg1_2 _lh_expr_fold_Func_0_2) ((map_lh__d4 ((expr_fold_lh__d2 _lh_expr_fold_arg1_2) _lh_expr_fold_arg2_2)) _lh_expr_fold_Func_1_2)));;
let rec occurs_lh__d2 _lh_occurs_arg1_0 =
  ((expr_fold_lh__d2 (fun dum_1 -> 
    or_lh__d2)) (fun x_2_2 -> 
    (x_2_2 = _lh_occurs_arg1_0)));;
let rec all_lh__d5 _lh_all_arg1_0 _lh_all_arg2_0 =
  (match _lh_all_arg2_0 with
    | `LH_N -> 
      true
    | `LH_C(_lh_all_LH_C_0_0, _lh_all_LH_C_1_0) -> 
      (if (_lh_all_arg1_0 _lh_all_LH_C_0_0) then
        ((all_lh__d5 _lh_all_arg1_0) _lh_all_LH_C_1_0)
      else
        false));;
let rec quant_lh__d2 _lh_quant_arg1_1 _lh_quant_arg2_1 _lh_quant_arg3_1 _lh_quant_arg4_1 =
  (let rec p_1 = (fun x_2_9 -> 
    ((_lh_quant_arg2_1 x_2_9) _lh_quant_arg3_1)) in
    ((_lh_quant_arg1_1 p_1) _lh_quant_arg4_1));;
let rec num_order_lh__d1 _lh_num_order_arg1_0 =
  (_lh_num_order_arg1_0 99);;
let rec lex_combine_lh__d1 _lh_lex_combine_arg1_0 _lh_lex_combine_arg2_0 =
  (match _lh_lex_combine_arg1_0 with
    | `Equal -> 
      _lh_lex_combine_arg2_0
    | `Greater -> 
      (`Greater)
    | `Less -> 
      (`Less)
    | `Unrelated -> 
      (`Unrelated));;
let rec lexico_lh__d1 _lh_lexico_arg1_0 _lh_lexico_arg2_0 =
  (match _lh_lexico_arg2_0 with
    | `LH_P2(_lh_lexico_LH_P2_0_0, _lh_lexico_LH_P2_1_0) -> 
      (match _lh_lexico_LH_P2_0_0 with
        | `LH_N -> 
          (match _lh_lexico_LH_P2_1_0 with
            | `LH_N -> 
              (`Equal)
            | `LH_C(_lh_lexico_LH_C_0_0, _lh_lexico_LH_C_1_0) -> 
              (`Less))
        | `LH_C(_lh_lexico_LH_C_0_1, _lh_lexico_LH_C_1_1) -> 
          (match _lh_lexico_LH_P2_1_0 with
            | `LH_N -> 
              (`Greater)
            | `LH_C(_lh_lexico_LH_C_0_2, _lh_lexico_LH_C_1_2) -> 
              ((lex_combine_lh__d1 (_lh_lexico_arg1_0 (`LH_P2(_lh_lexico_LH_C_0_1, _lh_lexico_LH_C_0_2)))) ((lexico_lh__d1 _lh_lexico_arg1_0) (`LH_P2(_lh_lexico_LH_C_1_1, _lh_lexico_LH_C_1_2)))))));;
let rec any_lh__d4 _lh_any_arg1_2 _lh_any_arg2_2 =
  (match _lh_any_arg2_2 with
    | `LH_N -> 
      false
    | `LH_C(_lh_any_LH_C_0_2, _lh_any_LH_C_1_2) -> 
      (if (_lh_any_arg1_2 _lh_any_LH_C_0_2) then
        true
      else
        ((any_lh__d4 _lh_any_arg1_2) _lh_any_LH_C_1_2)));;
let rec ltRewrite_lh__d3 _lh_ltRewrite_arg1_1 _lh_ltRewrite_arg2_1 _lh_ltRewrite_arg3_1 =
  ((_lh_ltRewrite_arg1_1 (`LH_P2(_lh_ltRewrite_arg2_1, _lh_ltRewrite_arg3_1))) = (`Less));;
let rec all_lh__d4 _lh_all_arg1_3 _lh_all_arg2_1 =
  (match _lh_all_arg2_1 with
    | `LH_N -> 
      true
    | `LH_C(_lh_all_LH_C_0_2, _lh_all_LH_C_1_2) -> 
      (if (_lh_all_arg1_3 _lh_all_LH_C_0_2) then
        ((all_lh__d4 _lh_all_arg1_3) _lh_all_LH_C_1_2)
      else
        false));;
let rec quant_lh__d1 _lh_quant_arg1_3 _lh_quant_arg2_3 _lh_quant_arg3_3 _lh_quant_arg4_3 =
  (let rec p_3 = (fun x_4_9 -> 
    ((_lh_quant_arg2_3 x_4_9) _lh_quant_arg3_3)) in
    ((_lh_quant_arg1_3 p_3) _lh_quant_arg4_3));;
let rec or_lh__d1 _lh_or_arg1_0 =
  (_lh_or_arg1_0 99);;
let rec map_lh__d3 f_1_1_0 ls_1_5 _lh_floatOutId_0_7 =
  (match ls_1_5 with
    | `LH_C(h_8_3, t_8_6) -> 
      (let rec _lh_or_LH_C_1_0 = ((map_lh__d3 f_1_1_0) t_8_6) in
        (let rec _lh_or_LH_C_0_0 = (f_1_1_0 h_8_3) in
          (if _lh_or_LH_C_0_0 then
            true
          else
            (or_lh__d1 _lh_or_LH_C_1_0))))
    | `LH_N -> 
      false);;
let rec expr_fold_lh__d1 _lh_expr_fold_arg1_3 _lh_expr_fold_arg2_3 _lh_expr_fold_arg3_3 =
  (match _lh_expr_fold_arg3_3 with
    | `Var(_lh_expr_fold_Var_0_3) -> 
      (_lh_expr_fold_arg2_3 _lh_expr_fold_Var_0_3)
    | `Func(_lh_expr_fold_Func_0_3, _lh_expr_fold_Func_1_3) -> 
      ((_lh_expr_fold_arg1_3 _lh_expr_fold_Func_0_3) ((map_lh__d3 ((expr_fold_lh__d1 _lh_expr_fold_arg1_3) _lh_expr_fold_arg2_3)) _lh_expr_fold_Func_1_3)));;
let rec occurs_lh__d1 _lh_occurs_arg1_1 =
  ((expr_fold_lh__d1 (fun dum_4 -> 
    or_lh__d1)) (fun x_5_0 -> 
    (x_5_0 = _lh_occurs_arg1_1)));;
let rec inList_lh__d1 _lh_inList_arg1_5 _lh_inList_arg2_2 =
  (_lh_inList_arg2_2 _lh_inList_arg1_5);;
let rec geRewrite_lh__d1 _lh_geRewrite_arg1_2 _lh_geRewrite_arg2_2 _lh_geRewrite_arg3_2 =
  ((inList_lh__d1 (_lh_geRewrite_arg1_2 (`LH_P2(_lh_geRewrite_arg2_2, _lh_geRewrite_arg3_2)))) (let rec _lh_inList_LH_C_1_3 = (let rec _lh_inList_LH_C_1_4 = (fun _lh_inList_arg1_6 -> 
    false) in
    (let rec _lh_inList_LH_C_0_3 = (`Equal) in
      (fun _lh_inList_arg1_7 -> 
        (if (_lh_inList_arg1_7 = _lh_inList_LH_C_0_3) then
          true
        else
          ((inList_lh__d1 _lh_inList_arg1_7) _lh_inList_LH_C_1_4))))) in
    (let rec _lh_inList_LH_C_0_4 = (`Greater) in
      (fun _lh_inList_arg1_8 -> 
        (if (_lh_inList_arg1_8 = _lh_inList_LH_C_0_4) then
          true
        else
          ((inList_lh__d1 _lh_inList_arg1_8) _lh_inList_LH_C_1_3))))));;
let rec quant_lh__d6 _lh_quant_arg1_4 _lh_quant_arg2_4 _lh_quant_arg3_4 _lh_quant_arg4_4 =
  (let rec p_4 = (fun x_6_0 -> 
    ((_lh_quant_arg2_4 x_6_0) _lh_quant_arg3_4)) in
    ((_lh_quant_arg1_4 p_4) _lh_quant_arg4_4));;
let rec ltRewrite_lh__d1 _lh_ltRewrite_arg1_3 _lh_ltRewrite_arg2_3 _lh_ltRewrite_arg3_3 =
  ((_lh_ltRewrite_arg1_3 (`LH_P2(_lh_ltRewrite_arg2_3, _lh_ltRewrite_arg3_3))) = (`Less));;
let rec all_lh__d3 _lh_all_arg1_6 _lh_all_arg2_4 =
  (match _lh_all_arg2_4 with
    | `LH_N -> 
      true
    | `LH_C(_lh_all_LH_C_0_4, _lh_all_LH_C_1_4) -> 
      (if (_lh_all_arg1_6 _lh_all_LH_C_0_4) then
        ((all_lh__d3 _lh_all_arg1_6) _lh_all_LH_C_1_4)
      else
        false));;
let rec quant_lh__d8 _lh_quant_arg1_5 _lh_quant_arg2_5 _lh_quant_arg3_5 _lh_quant_arg4_5 =
  (let rec p_5 = (fun x_6_1 -> 
    ((_lh_quant_arg2_5 x_6_1) _lh_quant_arg3_5)) in
    ((_lh_quant_arg1_5 p_5) _lh_quant_arg4_5));;
let rec inList_lh__d2 _lh_inList_arg1_1_2 _lh_inList_arg2_3 =
  (_lh_inList_arg2_3 _lh_inList_arg1_1_2);;
let rec geRewrite_lh__d2 _lh_geRewrite_arg1_3 _lh_geRewrite_arg2_3 _lh_geRewrite_arg3_3 =
  ((inList_lh__d2 (_lh_geRewrite_arg1_3 (`LH_P2(_lh_geRewrite_arg2_3, _lh_geRewrite_arg3_3)))) (let rec _lh_inList_LH_C_1_5 = (let rec _lh_inList_LH_C_1_6 = (fun _lh_inList_arg1_9 -> 
    false) in
    (let rec _lh_inList_LH_C_0_5 = (`Equal) in
      (fun _lh_inList_arg1_1_0 -> 
        (if (_lh_inList_arg1_1_0 = _lh_inList_LH_C_0_5) then
          true
        else
          ((inList_lh__d2 _lh_inList_arg1_1_0) _lh_inList_LH_C_1_6))))) in
    (let rec _lh_inList_LH_C_0_6 = (`Greater) in
      (fun _lh_inList_arg1_1_1 -> 
        (if (_lh_inList_arg1_1_1 = _lh_inList_LH_C_0_6) then
          true
        else
          ((inList_lh__d2 _lh_inList_arg1_1_1) _lh_inList_LH_C_1_5))))));;
let rec quant_lh__d3 _lh_quant_arg1_6 _lh_quant_arg2_6 _lh_quant_arg3_6 _lh_quant_arg4_6 =
  (let rec p_6 = (fun x_6_6 -> 
    ((_lh_quant_arg2_6 x_6_6) _lh_quant_arg3_6)) in
    ((_lh_quant_arg1_6 p_6) _lh_quant_arg4_6));;
let rec rpo_lh__d1 _lh_rpo_arg1_0 _lh_rpo_arg2_0 =
  (let rec rpo'_0 = (fun param_0 -> 
    (let rec _lh_matchIdent_1_0 = param_0 in
      (match _lh_matchIdent_1_0 with
        | `LH_P2(_lh_rpo_LH_P2_0_0, _lh_rpo_LH_P2_1_0) -> 
          (match _lh_rpo_LH_P2_0_0 with
            | `Var(_lh_rpo_Var_0_0) -> 
              (match _lh_rpo_LH_P2_1_0 with
                | `Var(_lh_rpo_Var_0_1) -> 
                  (if (_lh_rpo_Var_0_0 = _lh_rpo_Var_0_1) then
                    (`Equal)
                  else
                    (`Unrelated))
                | `Func(_lh_rpo_Func_0_0, _lh_rpo_Func_1_0) -> 
                  (if ((occurs_lh__d2 _lh_rpo_Var_0_0) (`Func(_lh_rpo_Func_0_0, _lh_rpo_Func_1_0))) then
                    (`Less)
                  else
                    (`Unrelated))
                | _ -> 
                  (failwith "msg"(`LH_C('e', (`LH_C('r', (`LH_C('r', (`LH_C('o', (`LH_C('r', (`LH_N)))))))))))))
            | `Func(_lh_rpo_Func_0_1, _lh_rpo_Func_1_1) -> 
              (match _lh_rpo_LH_P2_1_0 with
                | `Var(_lh_rpo_Var_0_2) -> 
                  (if ((occurs_lh__d1 _lh_rpo_Var_0_2) (`Func(_lh_rpo_Func_0_1, _lh_rpo_Func_1_1))) then
                    (`Greater)
                  else
                    (`Unrelated))
                | `Func(_lh_rpo_Func_0_2, _lh_rpo_Func_1_2) -> 
                  (let rec _lh_matchIdent_1_1 = (_lh_rpo_arg1_0 (let rec _lh_rank_order_LH_P2_1_0 = _lh_rpo_Func_0_2 in
                    (let rec _lh_rank_order_LH_P2_0_0 = _lh_rpo_Func_0_1 in
                      (fun _lh_rank_order_arg1_1 -> 
                        (num_order_lh__d1 (let rec _lh_num_order_LH_P2_1_0 = (_lh_rank_order_arg1_1 _lh_rank_order_LH_P2_1_0) in
                          (let rec _lh_num_order_LH_P2_0_0 = (_lh_rank_order_arg1_1 _lh_rank_order_LH_P2_0_0) in
                            (fun _lh_dummy_1 -> 
                              (if (_lh_num_order_LH_P2_0_0 > _lh_num_order_LH_P2_1_0) then
                                (fun _lh_rpo_Func_0_3 _lh_rpo_Func_0_4 _lh_rpo_Func_1_3 _lh_rpo_Func_1_4 _lh_rpo_arg2_1 rpo'_1 -> 
                                  (if ((((quant_lh__d4 all_lh__d3) (ltRewrite_lh__d2 rpo'_1)) (`Func(_lh_rpo_Func_0_4, _lh_rpo_Func_1_4))) _lh_rpo_Func_1_3) then
                                    (`Greater)
                                  else
                                    (if ((((quant_lh__d2 any_lh__d1) (geRewrite_lh__d1 rpo'_1)) (`Func(_lh_rpo_Func_0_4, _lh_rpo_Func_1_4))) _lh_rpo_Func_1_3) then
                                      (`Less)
                                    else
                                      (`Unrelated))))
                              else
                                (if (_lh_num_order_LH_P2_0_0 < _lh_num_order_LH_P2_1_0) then
                                  (fun _lh_rpo_Func_0_5 _lh_rpo_Func_0_6 _lh_rpo_Func_1_5 _lh_rpo_Func_1_6 _lh_rpo_arg2_2 rpo'_2 -> 
                                    (if ((((quant_lh__d3 all_lh__d2) (ltRewrite_lh__d1 rpo'_2)) (`Func(_lh_rpo_Func_0_5, _lh_rpo_Func_1_5))) _lh_rpo_Func_1_6) then
                                      (`Less)
                                    else
                                      (if ((((quant_lh__d1 any_lh__d2) (geRewrite_lh__d2 rpo'_2)) (`Func(_lh_rpo_Func_0_5, _lh_rpo_Func_1_5))) _lh_rpo_Func_1_6) then
                                        (`Greater)
                                      else
                                        (`Unrelated))))
                                else
                                  (fun _lh_rpo_Func_0_7 _lh_rpo_Func_0_8 _lh_rpo_Func_1_7 _lh_rpo_Func_1_8 _lh_rpo_arg2_3 rpo'_3 -> 
                                    ((_lh_rpo_arg2_3 rpo'_3) (let rec _lh_lex_ext_LH_P2_1_0 = (let rec _lh_lex_ext_Func_1_0 = _lh_rpo_Func_1_7 in
                                      (let rec _lh_lex_ext_Func_0_0 = _lh_rpo_Func_0_7 in
                                        (fun _lh_lex_ext_Func_0_1 _lh_lex_ext_Func_1_1 _lh_lex_ext_arg1_1 -> 
                                          (let rec estimate_0 = ((lexico_lh__d1 _lh_lex_ext_arg1_1) (`LH_P2(_lh_lex_ext_Func_1_1, _lh_lex_ext_Func_1_0))) in
                                            (let rec confirm_0 = (fun x_4_6 -> 
                                              (let rec _lh_matchIdent_1_2 = x_4_6 in
                                                (match _lh_matchIdent_1_2 with
                                                  | `Equal -> 
                                                    true
                                                  | `Greater -> 
                                                    ((((quant_lh__d6 all_lh__d4) (ltRewrite_lh__d3 _lh_lex_ext_arg1_1)) (`Func(_lh_lex_ext_Func_0_1, _lh_lex_ext_Func_1_1))) _lh_lex_ext_Func_1_0)
                                                  | `Less -> 
                                                    ((((quant_lh__d8 all_lh__d5) (ltRewrite_lh__d4 _lh_lex_ext_arg1_1)) (`Func(_lh_lex_ext_Func_0_0, _lh_lex_ext_Func_1_0))) _lh_lex_ext_Func_1_1)
                                                  | `Unrelated -> 
                                                    false))) in
                                              (if (confirm_0 estimate_0) then
                                                estimate_0
                                              else
                                                (if ((((quant_lh__d7 any_lh__d4) (geRewrite_lh__d3 _lh_lex_ext_arg1_1)) (`Func(_lh_lex_ext_Func_0_0, _lh_lex_ext_Func_1_0))) _lh_lex_ext_Func_1_1) then
                                                  (`Greater)
                                                else
                                                  (if ((((quant_lh__d5 any_lh__d3) (geRewrite_lh__d4 _lh_lex_ext_arg1_1)) (`Func(_lh_lex_ext_Func_0_1, _lh_lex_ext_Func_1_1))) _lh_lex_ext_Func_1_0) then
                                                    (`Less)
                                                  else
                                                    (`Unrelated))))))))) in
                                      (let rec _lh_lex_ext_LH_P2_0_0 = (let rec _lh_lex_ext_Func_1_2 = _lh_rpo_Func_1_8 in
                                        (let rec _lh_lex_ext_Func_0_2 = _lh_rpo_Func_0_8 in
                                          (fun _lh_lex_ext_LH_P2_1_1 _lh_lex_ext_arg1_2 -> 
                                            (((_lh_lex_ext_LH_P2_1_1 _lh_lex_ext_Func_0_2) _lh_lex_ext_Func_1_2) _lh_lex_ext_arg1_2)))) in
                                        (fun _lh_lex_ext_arg1_3 -> 
                                          ((_lh_lex_ext_LH_P2_0_0 _lh_lex_ext_LH_P2_1_0) _lh_lex_ext_arg1_3)))))))))))))))) in
                    ((((((_lh_matchIdent_1_1 _lh_rpo_Func_0_2) _lh_rpo_Func_0_1) _lh_rpo_Func_1_2) _lh_rpo_Func_1_1) _lh_rpo_arg2_0) rpo'_0))
                | _ -> 
                  (failwith "msg"(`LH_C('e', (`LH_C('r', (`LH_C('r', (`LH_C('o', (`LH_C('r', (`LH_N)))))))))))))
            | _ -> 
              (failwith "msg"(`LH_C('e', (`LH_C('r', (`LH_C('r', (`LH_C('o', (`LH_C('r', (`LH_N)))))))))))))
        | _ -> 
          (failwith "msg"(`LH_C('e', (`LH_C('r', (`LH_C('r', (`LH_C('o', (`LH_C('r', (`LH_N))))))))))))))) in
    rpo'_0);;
let rec mappend_lh__d1 xs_2 ys_1 =
  (xs_2 ys_1);;
let rec zip_lz_nl_lh__d1 xs_1_0 ys_5 =
  (match ys_5 with
    | `LH_C(hy_2, ty_2) -> 
      (((Lazy.force xs_1_0) hy_2) ty_2)
    | `LH_N -> 
      (`LH_N));;
let rec enumFrom_lh__d1 a_4 =
  (lazy (let rec tx_1 = (enumFrom_lh__d1 (a_4 + 1)) in
    (let rec hx_1 = a_4 in
      (fun hy_1 ty_1 -> 
        (`LH_C((`LH_P2(hx_1, hy_1)), ((zip_lz_nl_lh__d1 tx_1) ty_1)))))));;
let rec subterms_lh__d1 _lh_subterms_arg1_0 =
  (match _lh_subterms_arg1_0 with
    | `Var(_lh_subterms_Var_0_0) -> 
      (`LH_N)
    | `Func(_lh_subterms_Func_0_0, _lh_subterms_Func_1_0) -> 
      ((mappend_lh__d1 (let rec t_8_8 = (fun ys_3 -> 
        ys_3) in
        (let rec h_8_5 = (`LH_P2((`LH_N), (`Func(_lh_subterms_Func_0_0, _lh_subterms_Func_1_0)))) in
          (fun ys_4 -> 
            (`LH_C(h_8_5, ((mappend_lh__d1 t_8_8) ys_4))))))) (let rec _lh_listcomp_fun_5 = (fun _lh_listcomp_fun_para_5 -> 
        (match _lh_listcomp_fun_para_5 with
          | `LH_C(_lh_listcomp_fun_ls_h_5, _lh_listcomp_fun_ls_t_5) -> 
            (match _lh_listcomp_fun_ls_h_5 with
              | `LH_P2(_lh_subterms_LH_P2_0_0, _lh_subterms_LH_P2_1_0) -> 
                (let rec _lh_listcomp_fun_6 = (fun _lh_listcomp_fun_para_6 -> 
                  (match _lh_listcomp_fun_para_6 with
                    | `LH_C(_lh_listcomp_fun_ls_h_6, _lh_listcomp_fun_ls_t_6) -> 
                      (match _lh_listcomp_fun_ls_h_6 with
                        | `LH_P2(_lh_subterms_LH_P2_0_1, _lh_subterms_LH_P2_1_1) -> 
                          (`LH_C((`LH_P2((`LH_C(_lh_subterms_LH_P2_0_0, _lh_subterms_LH_P2_0_1)), _lh_subterms_LH_P2_1_1)), (_lh_listcomp_fun_6 _lh_listcomp_fun_ls_t_6)))
                        | _ -> 
                          (_lh_listcomp_fun_6 _lh_listcomp_fun_ls_t_6))
                    | `LH_N -> 
                      (_lh_listcomp_fun_5 _lh_listcomp_fun_ls_t_5))) in
                  (_lh_listcomp_fun_6 (subterms_lh__d1 _lh_subterms_LH_P2_1_0)))
              | _ -> 
                (_lh_listcomp_fun_5 _lh_listcomp_fun_ls_t_5))
          | `LH_N -> 
            (`LH_N))) in
        (_lh_listcomp_fun_5 ((zip_lz_nl_lh__d1 (enumFrom_lh__d1 0)) _lh_subterms_Func_1_0)))));;
let rec modify_lh__d1 _lh_modify_arg1_0 _lh_modify_arg2_0 _lh_modify_arg3_0 =
  (match _lh_modify_arg1_0 with
    | `LH_C(_lh_modify_LH_C_0_0, _lh_modify_LH_C_1_0) -> 
      (match _lh_modify_arg2_0 with
        | 0 -> 
          (`LH_C((_lh_modify_arg3_0 _lh_modify_LH_C_0_0), _lh_modify_LH_C_1_0))
        | _ -> 
          (`LH_C(_lh_modify_LH_C_0_0, (((modify_lh__d1 _lh_modify_LH_C_1_0) (_lh_modify_arg2_0 - 1)) _lh_modify_arg3_0))))
    | _ -> 
      (failwith "lh_default_error"));;
let rec replace_lh__d1 _lh_replace_arg1_0 _lh_replace_arg2_0 _lh_replace_arg3_0 =
  (match _lh_replace_arg2_0 with
    | `LH_N -> 
      _lh_replace_arg3_0
    | _ -> 
      (match _lh_replace_arg1_0 with
        | `Func(_lh_replace_Func_0_0, _lh_replace_Func_1_0) -> 
          (match _lh_replace_arg2_0 with
            | `LH_C(_lh_replace_LH_C_0_0, _lh_replace_LH_C_1_0) -> 
              (`Func(_lh_replace_Func_0_0, (((modify_lh__d1 _lh_replace_Func_1_0) _lh_replace_LH_C_0_0) (fun t_1_2 -> 
                (((replace_lh__d1 t_1_2) _lh_replace_LH_C_1_0) _lh_replace_arg3_0)))))
            | _ -> 
              (failwith "lh_default_error"))
        | _ -> 
          (failwith "lh_default_error")));;
let rec orElse_lh__d1 _lh_orElse_arg1_4 _lh_orElse_arg2_4 =
  (match _lh_orElse_arg1_4 with
    | `Nothing -> 
      _lh_orElse_arg2_4
    | _ -> 
      _lh_orElse_arg1_4);;
let rec foldr_lh__d2 f_5_4 i_4_8 ls_4 =
  (match ls_4 with
    | `LH_C(h_4_0, t_4_2) -> 
      ((f_5_4 h_4_0) (((foldr_lh__d2 f_5_4) i_4_8) t_4_2))
    | `LH_N -> 
      i_4_8);;
let rec first_ok_lh__d1 _lh_first_ok_arg1_0 =
  (((foldr_lh__d2 orElse_lh__d1) (`Nothing)) _lh_first_ok_arg1_0);;
let rec lift_lh__d2 _lh_lift_arg1_4 _lh_lift_arg2_4 =
  (match _lh_lift_arg2_4 with
    | `Nothing -> 
      (`Nothing)
    | `Just(_lh_lift_Just_0_4) -> 
      (`Just((_lh_lift_arg1_4 _lh_lift_Just_0_4))));;
let rec inside_lh__d1 _lh_inside_arg1_0 _lh_inside_arg2_0 =
  (first_ok_lh__d1 (let rec _lh_listcomp_fun_1 = (fun _lh_listcomp_fun_para_1 -> 
    (match _lh_listcomp_fun_para_1 with
      | `LH_C(_lh_listcomp_fun_ls_h_1, _lh_listcomp_fun_ls_t_1) -> 
        (match _lh_listcomp_fun_ls_h_1 with
          | `LH_P2(_lh_inside_LH_P2_0_0, _lh_inside_LH_P2_1_0) -> 
            (`LH_C(((lift_lh__d2 ((replace_lh__d1 _lh_inside_arg2_0) _lh_inside_LH_P2_0_0)) (_lh_inside_arg1_0 _lh_inside_LH_P2_1_0)), (_lh_listcomp_fun_1 _lh_listcomp_fun_ls_t_1)))
          | _ -> 
            (_lh_listcomp_fun_1 _lh_listcomp_fun_ls_t_1))
      | `LH_N -> 
        (`LH_N))) in
    (_lh_listcomp_fun_1 (subterms_lh__d1 _lh_inside_arg2_0))));;
let rec orElse_lh__d4 _lh_orElse_arg1_1 _lh_orElse_arg2_1 =
  (match _lh_orElse_arg1_1 with
    | `Nothing -> 
      _lh_orElse_arg2_1
    | _ -> 
      _lh_orElse_arg1_1);;
let rec orElseMap_lh__d3 _lh_orElseMap_arg1_6 _lh_orElseMap_arg2_6 _lh_orElseMap_arg3_6 =
  ((orElse_lh__d4 (_lh_orElseMap_arg1_6 _lh_orElseMap_arg3_6)) (_lh_orElseMap_arg2_6 _lh_orElseMap_arg3_6));;
let rec foldr_lh__d1 f_5_3 i_4_7 ls_3 =
  ((ls_3 f_5_3) i_4_7);;
let rec try_all_lh__d1 _lh_try_all_arg1_0 =
  (((foldr_lh__d1 orElseMap_lh__d3) (fun dum_3 -> 
    croak_lh__d1)) _lh_try_all_arg1_0);;
let rec lift_lh__d3 _lh_lift_arg1_3 _lh_lift_arg2_3 =
  (match _lh_lift_arg2_3 with
    | `Nothing -> 
      (`Nothing)
    | `Just(_lh_lift_Just_0_3) -> 
      (`Just((_lh_lift_arg1_3 _lh_lift_Just_0_3))));;
let rec expr_fold_lh__d4 _lh_expr_fold_arg1_0 _lh_expr_fold_arg2_0 _lh_expr_fold_arg3_0 =
  (match _lh_expr_fold_arg3_0 with
    | `Var(_lh_expr_fold_Var_0_0) -> 
      (_lh_expr_fold_arg2_0 _lh_expr_fold_Var_0_0)
    | `Func(_lh_expr_fold_Func_0_0, _lh_expr_fold_Func_1_0) -> 
      ((_lh_expr_fold_arg1_0 _lh_expr_fold_Func_0_0) ((map_lh__d7 ((expr_fold_lh__d4 _lh_expr_fold_arg1_0) _lh_expr_fold_arg2_0)) _lh_expr_fold_Func_1_0)));;
let rec assoc_lh__d1 _lh_assoc_arg1_0 _lh_assoc_arg2_0 =
  (let rec match_x_0 = (fun uv_0 -> 
    (let rec _lh_matchIdent_1_4 = uv_0 in
      (match _lh_matchIdent_1_4 with
        | `LH_P2(_lh_assoc_LH_P2_0_0, _lh_assoc_LH_P2_1_0) -> 
          ((maybeX_lh__d1 (_lh_assoc_arg2_0 = _lh_assoc_LH_P2_0_0)) _lh_assoc_LH_P2_1_0)))) in
    (first_ok_lh__d1 ((map_lh__d7 match_x_0) _lh_assoc_arg1_0)));;
let rec apply_lh__d1 _lh_apply_arg1_0 _lh_apply_arg2_0 =
  ((getOrElse_lh__d1 ((assoc_lh__d1 _lh_apply_arg1_0) _lh_apply_arg2_0)) (`Var(_lh_apply_arg2_0)));;
let rec sub_lh__d1 _lh_sub_arg1_0 _lh_sub_arg2_0 =
  (((expr_fold_lh__d4 (fun x_3_8 y_4 -> 
    (`Func(x_3_8, y_4)))) (apply_lh__d1 _lh_sub_arg2_0)) _lh_sub_arg1_0);;
let rec the_lh__d3 _lh_the_arg1_0 =
  (match _lh_the_arg1_0 with
    | `Just(_lh_the_Just_0_0) -> 
      _lh_the_Just_0_0
    | _ -> 
      (failwith "lh_default_error"));;
let rec exists_lh__d1 _lh_exists_arg1_0 =
  (match _lh_exists_arg1_0 with
    | `Nothing -> 
      false
    | _ -> 
      true);;
let rec prop2_lh__d1 _lh_prop2_arg1_0 _lh_prop2_arg2_0 _lh_prop2_arg3_0 =
  (match _lh_prop2_arg2_0 with
    | `Nothing -> 
      (`Nothing)
    | `Just(_lh_prop2_Just_0_0) -> 
      ((_lh_prop2_arg1_0 _lh_prop2_Just_0_0) _lh_prop2_arg3_0));;
let rec prop_fold_lh__d1 _lh_prop_fold_arg1_0 _lh_prop_fold_arg2_0 _lh_prop_fold_arg3_0 =
  (match _lh_prop_fold_arg3_0 with
    | `LH_N -> 
      (`Just(_lh_prop_fold_arg2_0))
    | `LH_C(_lh_prop_fold_LH_C_0_0, _lh_prop_fold_LH_C_1_0) -> 
      (((prop2_lh__d1 (prop_fold_lh__d1 _lh_prop_fold_arg1_0)) ((_lh_prop_fold_arg1_0 _lh_prop_fold_arg2_0) _lh_prop_fold_LH_C_0_0)) _lh_prop_fold_LH_C_1_0));;
let rec zip_lh__d1 xs_0 ys_0 =
  (match xs_0 with
    | `LH_C(hx_0, tx_0) -> 
      (match ys_0 with
        | `LH_C(hy_0, ty_0) -> 
          (`LH_C((`LH_P2(hx_0, hy_0)), ((zip_lh__d1 tx_0) ty_0)))
        | `LH_N -> 
          (`LH_N))
    | `LH_N -> 
      (`LH_N));;
let rec match'_lh__d1 _lh_match'_arg1_0 _lh_match'_arg2_0 =
  (match _lh_match'_arg2_0 with
    | `LH_P2(_lh_match'_LH_P2_0_0, _lh_match'_LH_P2_1_0) -> 
      (match _lh_match'_LH_P2_0_0 with
        | `Var(_lh_match'_Var_0_0) -> 
          (let rec u_0 = ((assoc_lh__d1 _lh_match'_arg1_0) _lh_match'_Var_0_0) in
            (if (not (exists_lh__d1 u_0)) then
              (succeed_lh__d1 (`LH_C((`LH_P2(_lh_match'_Var_0_0, _lh_match'_LH_P2_1_0)), _lh_match'_arg1_0)))
            else
              (if ((eqExpr_lh__d1 (the_lh__d3 u_0)) _lh_match'_LH_P2_1_0) then
                (succeed_lh__d1 _lh_match'_arg1_0)
              else
                croak_lh__d1)))
        | `Func(_lh_match'_Func_0_0, _lh_match'_Func_1_0) -> 
          (match _lh_match'_LH_P2_1_0 with
            | `Func(_lh_match'_Func_0_1, _lh_match'_Func_1_1) -> 
              (if (_lh_match'_Func_0_0 = _lh_match'_Func_0_1) then
                (((prop_fold_lh__d1 match'_lh__d1) _lh_match'_arg1_0) ((zip_lh__d1 _lh_match'_Func_1_0) _lh_match'_Func_1_1))
              else
                croak_lh__d1)
            | `Var(_lh_match'_Var_0_1) -> 
              croak_lh__d1
            | _ -> 
              (failwith "lh_default_error"))
        | _ -> 
          (failwith "lh_default_error"))
    | _ -> 
      (failwith "lh_default_error"));;
let rec match_lh__d1 _lh_match_arg1_0 _lh_match_arg2_0 =
  ((match'_lh__d1 (`LH_N)) (`LH_P2(_lh_match_arg1_0, _lh_match_arg2_0)));;
let rec rewrite_lh__d1 _lh_rewrite_arg1_0 _lh_floatOutId_0_5 =
  (match _lh_rewrite_arg1_0 with
    | `Eqn(_lh_rewrite_Eqn_0_0, _lh_rewrite_Eqn_1_0) -> 
      (match _lh_rewrite_Eqn_1_0 with
        | `LH_P2(_lh_rewrite_LH_P2_0_0, _lh_rewrite_LH_P2_1_0) -> 
          ((lift_lh__d3 (sub_lh__d1 _lh_rewrite_LH_P2_1_0)) ((match_lh__d1 _lh_rewrite_LH_P2_0_0) _lh_floatOutId_0_5))));;
let rec g_init_lh__d1 _lh_g_init_arg1_0 =
  (`LH_C(_lh_g_init_arg1_0, (`LH_N)));;
let rec g_first_lh__d1 _lh_g_first_arg1_0 =
  (match _lh_g_first_arg1_0 with
    | `LH_C(_lh_g_first_LH_C_0_0, _lh_g_first_LH_C_1_0) -> 
      (match _lh_g_first_LH_C_0_0 with
        | `Func(_lh_g_first_Func_0_0, _lh_g_first_Func_1_0) -> 
          (`Afunc(_lh_g_first_Func_0_0))
        | `Var(_lh_g_first_Var_0_0) -> 
          (`Avar)
        | _ -> 
          (failwith "lh_default_error"))
    | _ -> 
      (failwith "lh_default_error"));;
let rec mappend_lh__d2 xs_5 ys_2 =
  (match xs_5 with
    | `LH_C(h_3_9, t_4_1) -> 
      (`LH_C(h_3_9, ((mappend_lh__d2 t_4_1) ys_2)))
    | `LH_N -> 
      ys_2);;
let rec g_rest_lh__d1 _lh_g_rest_arg1_0 =
  (match _lh_g_rest_arg1_0 with
    | `LH_C(_lh_g_rest_LH_C_0_0, _lh_g_rest_LH_C_1_0) -> 
      (match _lh_g_rest_LH_C_0_0 with
        | `Func(_lh_g_rest_Func_0_0, _lh_g_rest_Func_1_0) -> 
          ((mappend_lh__d2 _lh_g_rest_Func_1_0) _lh_g_rest_LH_C_1_0)
        | `Var(_lh_g_rest_Var_0_0) -> 
          _lh_g_rest_LH_C_1_0
        | _ -> 
          (failwith "lh_default_error"))
    | _ -> 
      (failwith "lh_default_error"));;
let rec tail_lh__d1 ls_1_1 =
  (match ls_1_1 with
    | `LH_C(h_6_9, t_7_2) -> 
      t_7_2
    | `LH_N -> 
      (failwith "lh_default_error"));;
let rec g_skip_lh__d1 _lh_g_skip_arg1_0 =
  (tail_lh__d1 _lh_g_skip_arg1_0);;
let rec find'_lh__d1 _lh_find'_arg1_0 _lh_find'_arg2_0 =
  (match _lh_find'_arg2_0 with
    | `Return(_lh_find'_Return_0_0) -> 
      _lh_find'_Return_0_0
    | `Switch(_lh_find'_Switch_0_0, _lh_find'_Switch_1_0) -> 
      ((mappend_lh__d2 (((find_assoc_lh__d1 (g_first_lh__d1 _lh_find'_arg1_0)) (g_rest_lh__d1 _lh_find'_arg1_0)) _lh_find'_Switch_0_0)) ((find'_lh__d1 (g_skip_lh__d1 _lh_find'_arg1_0)) _lh_find'_Switch_1_0)))
and
find_assoc_lh__d1 _lh_find_assoc_arg1_0 _lh_find_assoc_arg2_0 _lh_find_assoc_arg3_0 =
  (match _lh_find_assoc_arg1_0 with
    | `Afunc(_lh_find_assoc_Afunc_0_0) -> 
      ((getOrElse_lh__d1 ((lift_lh__d6 (find'_lh__d1 _lh_find_assoc_arg2_0)) ((assoc_lh__d1 _lh_find_assoc_arg3_0) _lh_find_assoc_Afunc_0_0))) (`LH_N))
    | `Avar -> 
      (`LH_N));;
let rec find_lh__d1 _lh_find_arg1_0 _lh_find_arg2_0 =
  ((find'_lh__d1 (g_init_lh__d1 _lh_find_arg1_0)) _lh_find_arg2_0);;
let rec map_lh__d2 f_8_9 ls_1_0 _lh_floatOutId_0_2 _lh_floatOutId_1_0 =
  (match ls_1_0 with
    | `LH_C(h_6_7, t_7_0) -> 
      (let rec t_7_1 = ((map_lh__d2 f_8_9) t_7_0) in
        (let rec h_6_8 = (f_8_9 h_6_7) in
          ((_lh_floatOutId_0_2 h_6_8) (((foldr_lh__d1 _lh_floatOutId_0_2) _lh_floatOutId_1_0) t_7_1))))
    | `LH_N -> 
      _lh_floatOutId_1_0);;
let rec dnet_reduce_lh__d1 _lh_dnet_reduce_arg1_0 _lh_dnet_reduce_arg2_0 =
  ((try_all_lh__d1 ((map_lh__d2 rewrite_lh__d1) ((find_lh__d1 _lh_dnet_reduce_arg2_0) _lh_dnet_reduce_arg1_0))) _lh_dnet_reduce_arg2_0);;
let rec super_reduce_lh__d1 _lh_super_reduce_arg1_0 =
  (inside_lh__d1 (dnet_reduce_lh__d1 _lh_super_reduce_arg1_0));;
let rec result_lh__d1 _lh_result_arg1_0 =
  (_lh_result_arg1_0 99);;
let rec rank_lh__d1 _lh_rank_arg1_0 =
  (match _lh_rank_arg1_0 with
    | `LH_C(_lh_rank_LH_C_0_0, _lh_rank_LH_C_1_0) -> 
      (match _lh_rank_LH_C_0_0 with
        | 'E' -> 
          (match _lh_rank_LH_C_1_0 with
            | `LH_N -> 
              1
            | _ -> 
              (failwith "lh_default_error"))
        | '*' -> 
          (match _lh_rank_LH_C_1_0 with
            | `LH_N -> 
              2
            | _ -> 
              (failwith "lh_default_error"))
        | 'I' -> 
          (match _lh_rank_LH_C_1_0 with
            | `LH_N -> 
              3
            | _ -> 
              (failwith "lh_default_error"))
        | _ -> 
          (failwith "lh_default_error"))
    | _ -> 
      (failwith "lh_default_error"));;
let rec sumAux_lh__d1 ls_1_3 a_3 =
  (ls_1_3 a_3);;
let rec map_lh__d6 f_7_4 ls_8 _lh_floatOutId_0_1 =
  (match ls_8 with
    | `LH_C(h_5_5, t_5_8) -> 
      (let rec t_5_9 = ((map_lh__d6 f_7_4) t_5_8) in
        (let rec h_5_6 = (f_7_4 h_5_5) in
          ((sumAux_lh__d1 t_5_9) (_lh_floatOutId_0_1 + h_5_6))))
    | `LH_N -> 
      _lh_floatOutId_0_1);;
let rec expr_fold_lh__d3 _lh_expr_fold_arg1_1 _lh_expr_fold_arg2_1 _lh_expr_fold_arg3_1 =
  (match _lh_expr_fold_arg3_1 with
    | `Var(_lh_expr_fold_Var_0_1) -> 
      (_lh_expr_fold_arg2_1 _lh_expr_fold_Var_0_1)
    | `Func(_lh_expr_fold_Func_0_1, _lh_expr_fold_Func_1_1) -> 
      ((_lh_expr_fold_arg1_1 _lh_expr_fold_Func_0_1) ((map_lh__d6 ((expr_fold_lh__d3 _lh_expr_fold_arg1_1) _lh_expr_fold_arg2_1)) _lh_expr_fold_Func_1_1)));;
let rec sum_lh__d1 ls_7 =
  ((sumAux_lh__d1 ls_7) 0);;
let rec cost_lh__d1 _lh_cost_arg1_0 =
  (match _lh_cost_arg1_0 with
    | `LH_P2(_lh_cost_LH_P2_0_0, _lh_cost_LH_P2_1_0) -> 
      (let rec size_0 = (fun _lh_size_arg1_0 -> 
        (((expr_fold_lh__d3 (fun x_9 a_0 -> 
          ((sum_lh__d1 a_0) + 1))) (fun dum_0 -> 
          0)) _lh_size_arg1_0)) in
        ((size_0 _lh_cost_LH_P2_0_0) + (size_0 _lh_cost_LH_P2_1_0))));;
let rec empty_net_lh__d1 =
  (`Return((`LH_N)));;
let rec addby_lh__d1 _lh_addby_arg1_0 _lh_addby_arg2_0 _lh_addby_arg3_0 =
  (let rec insert_0 = (fun x_3_1 ls_6 -> 
    (let rec _lh_matchIdent_4 = ls_6 in
      (match _lh_matchIdent_4 with
        | `LH_N -> 
          (`LH_C(x_3_1, (`LH_N)))
        | `LH_C(_lh_addby_LH_C_0_0, _lh_addby_LH_C_1_0) -> 
          (if ((_lh_addby_arg1_0 x_3_1) <= (_lh_addby_arg1_0 _lh_addby_LH_C_0_0)) then
            (`LH_C(x_3_1, (`LH_C(_lh_addby_LH_C_0_0, _lh_addby_LH_C_1_0))))
          else
            (`LH_C(_lh_addby_LH_C_0_0, ((insert_0 x_3_1) _lh_addby_LH_C_1_0))))))) in
    (((foldr_lh__d6 insert_0) _lh_addby_arg3_0) _lh_addby_arg2_0));;
let rec item_cost_lh__d1 _lh_item_cost_arg1_0 =
  (match _lh_item_cost_arg1_0 with
    | `Item(_lh_item_cost_Item_0_0, _lh_item_cost_Item_1_0) -> 
      _lh_item_cost_Item_0_0);;
let rec mk_item_lh__d1 _lh_mk_item_arg1_0 _lh_mk_item_arg2_0 =
  (`Item((_lh_mk_item_arg1_0 _lh_mk_item_arg2_0), _lh_mk_item_arg2_0));;
let rec map_lh__d5 f_1_0_7 ls_1_4 _lh_floatOutId_0_6 _lh_floatOutId_1_2 =
  (match ls_1_4 with
    | `LH_C(h_8_1, t_8_4) -> 
      (let rec t_8_5 = ((map_lh__d5 f_1_0_7) t_8_4) in
        (let rec h_8_2 = (f_1_0_7 h_8_1) in
          ((_lh_floatOutId_0_6 h_8_2) (((foldr_lh__d6 _lh_floatOutId_0_6) _lh_floatOutId_1_2) t_8_5))))
    | `LH_N -> 
      _lh_floatOutId_1_2);;
let rec add_agenda_lh__d1 _lh_add_agenda_arg1_0 _lh_add_agenda_arg2_0 =
  ((addby_lh__d1 item_cost_lh__d1) ((map_lh__d5 (mk_item_lh__d1 _lh_add_agenda_arg1_0)) _lh_add_agenda_arg2_0));;
let rec snd_lh__d1 _lh_snd_arg1_0 =
  (match _lh_snd_arg1_0 with
    | `LH_P2(_lh_snd_LH_P2_0_0, _lh_snd_LH_P2_1_0) -> 
      _lh_snd_LH_P2_1_0);;
let rec fst_lh__d3 _lh_fst_arg1_2 =
  (match _lh_fst_arg1_2 with
    | `LH_P2(_lh_fst_LH_P2_0_2, _lh_fst_LH_P2_1_2) -> 
      _lh_fst_LH_P2_0_2);;
let rec reducible_lh__d1 _lh_reducible_arg1_0 _lh_reducible_arg2_0 =
  (match _lh_reducible_arg2_0 with
    | `Eqn(_lh_reducible_Eqn_0_0, _lh_reducible_Eqn_1_0) -> 
      (match _lh_reducible_Eqn_1_0 with
        | `LH_P2(_lh_reducible_LH_P2_0_0, _lh_reducible_LH_P2_1_0) -> 
          ((exists_lh__d1 (_lh_reducible_arg1_0 _lh_reducible_LH_P2_0_0)) || (exists_lh__d1 (_lh_reducible_arg1_0 _lh_reducible_LH_P2_1_0)))));;
let rec reduce1_lh__d1 _lh_reduce1_arg1_0 =
  (inside_lh__d1 (rewrite_lh__d1 _lh_reduce1_arg1_0));;
let rec update_alist_lh__d1 _lh_update_alist_arg1_0 _lh_update_alist_arg2_0 _lh_update_alist_arg3_0 =
  (let rec upd_0 = (fun ls_1_8 -> 
    (let rec _lh_matchIdent_1_6 = ls_1_8 in
      (match _lh_matchIdent_1_6 with
        | `LH_N -> 
          (`LH_C((`LH_P2(_lh_update_alist_arg1_0, (_lh_update_alist_arg2_0 _lh_update_alist_arg3_0))), (`LH_N)))
        | `LH_C(_lh_update_alist_LH_C_0_0, _lh_update_alist_LH_C_1_0) -> 
          (match _lh_update_alist_LH_C_0_0 with
            | `LH_P2(_lh_update_alist_LH_P2_0_0, _lh_update_alist_LH_P2_1_0) -> 
              (if (_lh_update_alist_arg1_0 = _lh_update_alist_LH_P2_0_0) then
                (`LH_C((`LH_P2(_lh_update_alist_arg1_0, (_lh_update_alist_arg2_0 _lh_update_alist_LH_P2_1_0))), _lh_update_alist_LH_C_1_0))
              else
                (`LH_C((`LH_P2(_lh_update_alist_LH_P2_0_0, _lh_update_alist_LH_P2_1_0)), (upd_0 _lh_update_alist_LH_C_1_0)))))))) in
    upd_0);;
let rec is_switch_lh__d1 _lh_is_switch_arg1_0 =
  (match _lh_is_switch_arg1_0 with
    | `Switch(_lh_is_switch_Switch_0_0, _lh_is_switch_Switch_1_0) -> 
      true
    | `Return(_lh_is_switch_Return_0_0) -> 
      false);;
let rec thread_lh__d1 _lh_thread_arg1_0 _lh_thread_arg2_0 _lh_thread_arg3_0 =
  (match _lh_thread_arg1_0 with
    | `LH_C(_lh_thread_LH_C_0_0, _lh_thread_LH_C_1_0) -> 
      (match _lh_thread_LH_C_0_0 with
        | `Afunc(_lh_thread_Afunc_0_0) -> 
          (match _lh_thread_arg3_0 with
            | `Switch(_lh_thread_Switch_0_0, _lh_thread_Switch_1_0) -> 
              (`Switch(((((update_alist_lh__d1 _lh_thread_Afunc_0_0) ((thread_lh__d1 _lh_thread_LH_C_1_0) _lh_thread_arg2_0)) empty_net_lh__d1) _lh_thread_Switch_0_0), _lh_thread_Switch_1_0))
            | `Return(_lh_thread_Return_0_0) -> 
              (((thread_lh__d1 (`LH_C((`Afunc(_lh_thread_Afunc_0_0)), _lh_thread_LH_C_1_0))) _lh_thread_arg2_0) (`Switch((`LH_N), (`Return(_lh_thread_Return_0_0))))))
        | `Avar -> 
          (match _lh_thread_arg3_0 with
            | `Switch(_lh_thread_Switch_0_1, _lh_thread_Switch_1_1) -> 
              (`Switch(_lh_thread_Switch_0_1, (((thread_lh__d1 _lh_thread_LH_C_1_0) _lh_thread_arg2_0) _lh_thread_Switch_1_1)))
            | `Return(_lh_thread_Return_0_1) -> 
              (let rec d'_0 = (((thread_lh__d1 _lh_thread_LH_C_1_0) _lh_thread_arg2_0) (`Return(_lh_thread_Return_0_1))) in
                (if (is_switch_lh__d1 d'_0) then
                  (`Switch((`LH_N), d'_0))
                else
                  d'_0))))
    | `LH_N -> 
      (match _lh_thread_arg3_0 with
        | `Return(_lh_thread_Return_0_2) -> 
          (`Return((`LH_C(_lh_thread_arg2_0, _lh_thread_Return_0_2))))));;
let rec lhs_lh__d1 _lh_lhs_arg1_0 =
  (match _lh_lhs_arg1_0 with
    | `Eqn(_lh_lhs_Eqn_0_0, _lh_lhs_Eqn_1_0) -> 
      (match _lh_lhs_Eqn_1_0 with
        | `LH_P2(_lh_lhs_LH_P2_0_0, _lh_lhs_LH_P2_1_0) -> 
          _lh_lhs_LH_P2_0_0));;
let rec preorder_lh__d1 _lh_preorder_arg1_0 =
  (if ((eqListExpr_lh__d1 _lh_preorder_arg1_0) (`LH_N)) then
    (`LH_N)
  else
    ((mappend_lh__d2 (`LH_C((g_first_lh__d1 _lh_preorder_arg1_0), (`LH_N)))) (preorder_lh__d1 (g_rest_lh__d1 _lh_preorder_arg1_0))));;
let rec add_eqn_lh__d1 _lh_add_eqn_arg1_0 _lh_add_eqn_arg2_0 =
  (((thread_lh__d1 (preorder_lh__d1 (g_init_lh__d1 (lhs_lh__d1 _lh_add_eqn_arg2_0)))) _lh_add_eqn_arg2_0) _lh_add_eqn_arg1_0);;
let rec eqno_lh__d1 _lh_eqno_arg1_0 =
  (match _lh_eqno_arg1_0 with
    | `Eqn(_lh_eqno_Eqn_0_0, _lh_eqno_Eqn_1_0) -> 
      (match _lh_eqno_Eqn_1_0 with
        | `LH_P2(_lh_eqno_LH_P2_0_0, _lh_eqno_LH_P2_1_0) -> 
          _lh_eqno_Eqn_0_0));;
let rec map_dnet_lh__d1 _lh_map_dnet_arg1_0 _lh_map_dnet_arg2_0 =
  (match _lh_map_dnet_arg2_0 with
    | `Switch(_lh_map_dnet_Switch_0_0, _lh_map_dnet_Switch_1_0) -> 
      (`Switch(((map_lh__d7 ((cross_lh__d2 (fun x_2_6 -> 
        x_2_6)) (map_dnet_lh__d1 _lh_map_dnet_arg1_0))) _lh_map_dnet_Switch_0_0), ((map_dnet_lh__d1 _lh_map_dnet_arg1_0) _lh_map_dnet_Switch_1_0)))
    | `Return(_lh_map_dnet_Return_0_0) -> 
      (`Return((_lh_map_dnet_arg1_0 _lh_map_dnet_Return_0_0))));;
let rec filter_lh__d1 f_1_1_1 ls_1_6 =
  (match ls_1_6 with
    | `LH_C(h_8_4, t_8_7) -> 
      (if (f_1_1_1 h_8_4) then
        (`LH_C(h_8_4, ((filter_lh__d1 f_1_1_1) t_8_7)))
      else
        ((filter_lh__d1 f_1_1_1) t_8_7))
    | `LH_N -> 
      (`LH_N));;
let rec delete_eqns_lh__d1 _lh_delete_eqns_arg1_0 =
  (let rec ok_0 = (fun e_0 -> 
    (not ((inList_lh__d4 (eqno_lh__d1 e_0)) _lh_delete_eqns_arg1_0))) in
    (map_dnet_lh__d1 (filter_lh__d1 ok_0)));;
let rec concat_lh__d1 lss_0 =
  (match lss_0 with
    | `LH_C(h_9_8, t_1_0_1) -> 
      ((mappend_lh__d2 h_9_8) (concat_lh__d1 t_1_0_1))
    | `LH_N -> 
      (`LH_N));;
let rec box_lh__d1 _lh_box_arg1_0 =
  (`LH_C(_lh_box_arg1_0, (`LH_N)));;
let rec listify_lh__d1 _lh_listify_arg1_0 =
  ((getOrElse_lh__d1 ((lift_lh__d6 box_lh__d1) _lh_listify_arg1_0)) (`LH_N));;
let rec sift_lh__d1 _lh_sift_arg1_0 =
  ((fun _lh_funcomp_x_3_3 -> 
    (concat_lh__d1 ((map_lh__d7 listify_lh__d1) _lh_funcomp_x_3_3))) _lh_sift_arg1_0);;
let rec resolve_lh__d1 _lh_resolve_arg1_0 _lh_resolve_arg2_0 =
  (let rec lhs'_0 = ((simplify_lh__d1 _lh_resolve_arg1_0) (fst_lh__d3 _lh_resolve_arg2_0)) in
    (let rec rhs'_0 = ((simplify_lh__d1 _lh_resolve_arg1_0) (snd_lh__d1 _lh_resolve_arg2_0)) in
      (if ((eqExpr_lh__d1 lhs'_0) rhs'_0) then
        croak_lh__d1
      else
        (succeed_lh__d1 (`LH_P2(lhs'_0, rhs'_0))))));;
let rec eqpr_lh__d1 _lh_eqpr_arg1_0 =
  (match _lh_eqpr_arg1_0 with
    | `Eqn(_lh_eqpr_Eqn_0_0, _lh_eqpr_Eqn_1_0) -> 
      _lh_eqpr_Eqn_1_0);;
let rec mk_crit_lh__d1 _lh_mk_crit_arg1_0 _lh_mk_crit_arg2_0 _lh_mk_crit_arg3_0 =
  (match _lh_mk_crit_arg1_0 with
    | `Eqn(_lh_mk_crit_Eqn_0_0, _lh_mk_crit_Eqn_1_0) -> 
      (match _lh_mk_crit_Eqn_1_0 with
        | `LH_P2(_lh_mk_crit_LH_P2_0_0, _lh_mk_crit_LH_P2_1_0) -> 
          (match _lh_mk_crit_arg2_0 with
            | `Eqn(_lh_mk_crit_Eqn_0_1, _lh_mk_crit_Eqn_1_1) -> 
              (match _lh_mk_crit_Eqn_1_1 with
                | `LH_P2(_lh_mk_crit_LH_P2_0_1, _lh_mk_crit_LH_P2_1_1) -> 
                  (match _lh_mk_crit_arg3_0 with
                    | `LH_P2(_lh_mk_crit_LH_P2_0_2, _lh_mk_crit_LH_P2_1_2) -> 
                      (`LH_P2(((sub_lh__d1 (((replace_lh__d1 _lh_mk_crit_LH_P2_0_1) _lh_mk_crit_LH_P2_0_2) _lh_mk_crit_LH_P2_1_0)) _lh_mk_crit_LH_P2_1_2), ((sub_lh__d1 _lh_mk_crit_LH_P2_1_1) _lh_mk_crit_LH_P2_1_2))))))));;
let rec comp_sub_lh__d1 _lh_comp_sub_arg1_0 _lh_comp_sub_arg2_0 =
  ((mappend_lh__d2 (let rec _lh_listcomp_fun_2 = (fun _lh_listcomp_fun_para_2 -> 
    (match _lh_listcomp_fun_para_2 with
      | `LH_C(_lh_listcomp_fun_ls_h_2, _lh_listcomp_fun_ls_t_2) -> 
        (match _lh_listcomp_fun_ls_h_2 with
          | `LH_P2(_lh_comp_sub_LH_P2_0_0, _lh_comp_sub_LH_P2_1_0) -> 
            (`LH_C((`LH_P2(_lh_comp_sub_LH_P2_0_0, ((sub_lh__d1 _lh_comp_sub_LH_P2_1_0) _lh_comp_sub_arg1_0))), (_lh_listcomp_fun_2 _lh_listcomp_fun_ls_t_2)))
          | _ -> 
            (_lh_listcomp_fun_2 _lh_listcomp_fun_ls_t_2))
      | `LH_N -> 
        (`LH_N))) in
    (_lh_listcomp_fun_2 _lh_comp_sub_arg2_0))) _lh_comp_sub_arg1_0);;
let rec unify'_lh__d1 _lh_unify'_arg1_0 _lh_unify'_arg2_0 =
  (match _lh_unify'_arg2_0 with
    | `LH_P2(_lh_unify'_LH_P2_0_0, _lh_unify'_LH_P2_1_0) -> 
      (match _lh_unify'_LH_P2_0_0 with
        | `Var(_lh_unify'_Var_0_0) -> 
          (((univar_lh__d1 _lh_unify'_arg1_0) _lh_unify'_Var_0_0) _lh_unify'_LH_P2_1_0)
        | `Func(_lh_unify'_Func_0_0, _lh_unify'_Func_1_0) -> 
          (match _lh_unify'_LH_P2_1_0 with
            | `Var(_lh_unify'_Var_0_1) -> 
              (((univar_lh__d1 _lh_unify'_arg1_0) _lh_unify'_Var_0_1) (`Func(_lh_unify'_Func_0_0, _lh_unify'_Func_1_0)))
            | `Func(_lh_unify'_Func_0_1, _lh_unify'_Func_1_1) -> 
              (if (_lh_unify'_Func_0_0 = _lh_unify'_Func_0_1) then
                (((prop_fold_lh__d1 unify'_lh__d1) _lh_unify'_arg1_0) ((zip_lh__d1 _lh_unify'_Func_1_0) _lh_unify'_Func_1_1))
              else
                croak_lh__d1))))
and
univar_lh__d1 _lh_univar_arg1_0 _lh_univar_arg2_0 _lh_univar_arg3_0 =
  (let rec t_4_3 = ((assoc_lh__d1 _lh_univar_arg1_0) _lh_univar_arg2_0) in
    (if (exists_lh__d1 t_4_3) then
      ((unify'_lh__d1 _lh_univar_arg1_0) (`LH_P2((the_lh__d3 t_4_3), _lh_univar_arg3_0)))
    else
      (if ((eqExpr_lh__d1 _lh_univar_arg3_0) (`Var(_lh_univar_arg2_0))) then
        (succeed_lh__d1 _lh_univar_arg1_0)
      else
        (let rec u'_0 = ((sub_lh__d1 _lh_univar_arg3_0) _lh_univar_arg1_0) in
          (if (not ((occurs_lh__d2 _lh_univar_arg2_0) u'_0)) then
            (succeed_lh__d1 ((comp_sub_lh__d1 (`LH_C((`LH_P2(_lh_univar_arg2_0, u'_0)), (`LH_N)))) _lh_univar_arg1_0))
          else
            croak_lh__d1)))));;
let rec unify_lh__d1 _lh_unify_arg1_0 _lh_unify_arg2_0 =
  ((unify'_lh__d1 (`LH_N)) (`LH_P2(_lh_unify_arg1_0, _lh_unify_arg2_0)));;
let rec pair_lh__d1 _lh_pair_arg1_0 _lh_pair_arg2_0 =
  (`LH_P2(_lh_pair_arg1_0, _lh_pair_arg2_0));;
let rec superpose_lh__d1 _lh_superpose_arg1_0 _lh_superpose_arg2_0 =
  (sift_lh__d1 (let rec _lh_listcomp_fun_0 = (fun _lh_listcomp_fun_para_0 -> 
    (match _lh_listcomp_fun_para_0 with
      | `LH_C(_lh_listcomp_fun_ls_h_0, _lh_listcomp_fun_ls_t_0) -> 
        (match _lh_listcomp_fun_ls_h_0 with
          | `LH_P2(_lh_superpose_LH_P2_0_0, _lh_superpose_LH_P2_1_0) -> 
            (`LH_C(((lift_lh__d6 (pair_lh__d1 _lh_superpose_LH_P2_0_0)) ((unify_lh__d1 _lh_superpose_arg1_0) _lh_superpose_LH_P2_1_0)), (_lh_listcomp_fun_0 _lh_listcomp_fun_ls_t_0)))
          | _ -> 
            (_lh_listcomp_fun_0 _lh_listcomp_fun_ls_t_0))
      | `LH_N -> 
        (`LH_N))) in
    (_lh_listcomp_fun_0 (subterms_lh__d1 _lh_superpose_arg2_0))));;
let rec strict_super_lh__d1 _lh_strict_super_arg1_0 _lh_strict_super_arg2_0 =
  (match _lh_strict_super_arg2_0 with
    | `Func(_lh_strict_super_Func_0_0, _lh_strict_super_Func_1_0) -> 
      (let rec _lh_listcomp_fun_7 = (fun _lh_listcomp_fun_para_7 -> 
        (match _lh_listcomp_fun_para_7 with
          | `LH_C(_lh_listcomp_fun_ls_h_7, _lh_listcomp_fun_ls_t_7) -> 
            (match _lh_listcomp_fun_ls_h_7 with
              | `LH_P2(_lh_strict_super_LH_P2_0_0, _lh_strict_super_LH_P2_1_0) -> 
                (let rec _lh_listcomp_fun_8 = (fun _lh_listcomp_fun_para_8 -> 
                  (match _lh_listcomp_fun_para_8 with
                    | `LH_C(_lh_listcomp_fun_ls_h_8, _lh_listcomp_fun_ls_t_8) -> 
                      (match _lh_listcomp_fun_ls_h_8 with
                        | `LH_P2(_lh_strict_super_LH_P2_0_1, _lh_strict_super_LH_P2_1_1) -> 
                          (`LH_C((`LH_P2((`LH_C(_lh_strict_super_LH_P2_0_0, _lh_strict_super_LH_P2_0_1)), _lh_strict_super_LH_P2_1_1)), (_lh_listcomp_fun_8 _lh_listcomp_fun_ls_t_8)))
                        | _ -> 
                          (_lh_listcomp_fun_8 _lh_listcomp_fun_ls_t_8))
                    | `LH_N -> 
                      (_lh_listcomp_fun_7 _lh_listcomp_fun_ls_t_7))) in
                  (_lh_listcomp_fun_8 ((superpose_lh__d1 _lh_strict_super_arg1_0) _lh_strict_super_LH_P2_1_0)))
              | _ -> 
                (_lh_listcomp_fun_7 _lh_listcomp_fun_ls_t_7))
          | `LH_N -> 
            (`LH_N))) in
        (_lh_listcomp_fun_7 ((zip_lz_nl_lh__d1 (enumFrom_lh__d1 0)) _lh_strict_super_Func_1_0)))
    | _ -> 
      (failwith "lh_default_error"));;
let rec crit_pairs_lh__d1 _lh_crit_pairs_arg1_0 _lh_crit_pairs_arg2_0 =
  ((mappend_lh__d2 ((map_lh__d7 ((mk_crit_lh__d1 _lh_crit_pairs_arg1_0) _lh_crit_pairs_arg2_0)) ((superpose_lh__d1 (lhs_lh__d1 _lh_crit_pairs_arg1_0)) (lhs_lh__d1 _lh_crit_pairs_arg2_0)))) ((map_lh__d7 ((mk_crit_lh__d1 _lh_crit_pairs_arg2_0) _lh_crit_pairs_arg1_0)) ((strict_super_lh__d1 (lhs_lh__d1 _lh_crit_pairs_arg2_0)) (lhs_lh__d1 _lh_crit_pairs_arg1_0))));;
let rec stand_lh__d1 _lh_stand_arg1_0 =
  ((expr_fold_lh__d4 (fun x_3_6 y_3 -> 
    (`Func(x_3_6, y_3)))) (fun v_0 -> 
    (`Var(((mappend_lh__d2 v_0) _lh_stand_arg1_0)))));;
let rec stand_eqn_lh__d1 _lh_stand_eqn_arg1_0 _lh_stand_eqn_arg2_0 =
  (match _lh_stand_eqn_arg2_0 with
    | `Eqn(_lh_stand_eqn_Eqn_0_0, _lh_stand_eqn_Eqn_1_0) -> 
      (match _lh_stand_eqn_Eqn_1_0 with
        | `LH_P2(_lh_stand_eqn_LH_P2_0_0, _lh_stand_eqn_LH_P2_1_0) -> 
          (`Eqn(_lh_stand_eqn_Eqn_0_0, (`LH_P2(((stand_lh__d1 _lh_stand_eqn_arg1_0) _lh_stand_eqn_LH_P2_0_0), ((stand_lh__d1 _lh_stand_eqn_arg1_0) _lh_stand_eqn_LH_P2_1_0)))))));;
let rec all_crit_pairs_lh__d1 _lh_all_crit_pairs_arg1_0 _lh_all_crit_pairs_arg2_0 =
  (let rec eqn'_0 = ((stand_eqn_lh__d1 (`LH_C('1', (`LH_N)))) _lh_all_crit_pairs_arg1_0) in
    (let rec eqn''_0 = ((stand_eqn_lh__d1 (`LH_C('2', (`LH_N)))) _lh_all_crit_pairs_arg1_0) in
      (let rec theory''_0 = ((map_lh__d7 (stand_eqn_lh__d1 (`LH_C('2', (`LH_N))))) _lh_all_crit_pairs_arg2_0) in
        ((mappend_lh__d2 ((map_lh__d7 ((mk_crit_lh__d1 eqn'_0) eqn''_0)) ((strict_super_lh__d1 (lhs_lh__d1 eqn'_0)) (lhs_lh__d1 eqn''_0)))) (concat_lh__d1 ((map_lh__d7 (crit_pairs_lh__d1 eqn'_0)) theory''_0))))));;
let rec process1_lh__d1 _lh_process1_arg1_0 _lh_process1_arg2_0 _lh_process1_arg3_0 _lh_process1_arg4_0 _lh_process1_arg5_0 _lh_process1_arg6_0 =
  (((((_lh_process1_arg6_0 _lh_process1_arg1_0) _lh_process1_arg2_0) _lh_process1_arg3_0) _lh_process1_arg4_0) _lh_process1_arg5_0);;
let rec preprocess_lh__d1 _lh_preprocess_arg1_0 _lh_preprocess_arg2_0 _lh_floatOutId_0_3 _lh_floatOutId_1_1 _lh_floatOutId_2_0 _lh_floatOutId_3_0 _lh_floatOutId_4_0 =
  (match _lh_preprocess_arg2_0 with
    | `LH_P2(_lh_preprocess_LH_P2_0_0, _lh_preprocess_LH_P2_1_0) -> 
      (let rec _lh_process1_LH_P2_1_0 = ((simplify_lh__d1 (super_reduce_lh__d1 _lh_preprocess_arg1_0)) _lh_preprocess_LH_P2_1_0) in
        (let rec _lh_process1_LH_P2_0_0 = ((simplify_lh__d1 (super_reduce_lh__d1 _lh_preprocess_arg1_0)) _lh_preprocess_LH_P2_0_0) in
          (let rec o_0 = ((fst_lh__d3 _lh_floatOutId_0_3) (`LH_P2(_lh_process1_LH_P2_0_0, _lh_process1_LH_P2_1_0))) in
            (let rec _lh_matchIdent_6 = o_0 in
              (match _lh_matchIdent_6 with
                | `Unrelated -> 
                  (((((process_lh__d1 _lh_floatOutId_0_3) _lh_floatOutId_1_1) _lh_floatOutId_2_0) _lh_floatOutId_3_0) (((add_agenda_lh__d1 (fun dum_2 -> 
                    1000)) (`LH_C((`LH_P2(_lh_process1_LH_P2_0_0, _lh_process1_LH_P2_1_0)), (`LH_N)))) _lh_floatOutId_4_0))
                | `Equal -> 
                  (((((process_lh__d1 _lh_floatOutId_0_3) _lh_floatOutId_1_1) _lh_floatOutId_2_0) _lh_floatOutId_3_0) _lh_floatOutId_4_0)
                | _ -> 
                  (let rec eqn_0 = (let rec _lh_matchIdent_7 = o_0 in
                    (match _lh_matchIdent_7 with
                      | `Greater -> 
                        (`Eqn(_lh_floatOutId_1_1, (`LH_P2(_lh_process1_LH_P2_0_0, _lh_process1_LH_P2_1_0))))
                      | `Less -> 
                        (`Eqn(_lh_floatOutId_1_1, (`LH_P2(_lh_process1_LH_P2_1_0, _lh_process1_LH_P2_0_0)))))) in
                    ((((((process2_lh__d1 _lh_floatOutId_0_3) (_lh_floatOutId_1_1 + 1)) _lh_floatOutId_2_0) _lh_floatOutId_3_0) _lh_floatOutId_4_0) eqn_0))))))))
and
process2_lh__d1 _lh_process2_arg1_0 _lh_process2_arg2_0 _lh_process2_arg3_0 _lh_process2_arg4_0 _lh_process2_arg5_0 _lh_process2_arg6_0 =
  (let rec _lh_matchIdent_9 = ((split_lh__d1 (reducible_lh__d1 (reduce1_lh__d1 _lh_process2_arg6_0))) _lh_process2_arg3_0) in
    (((((_lh_matchIdent_9 _lh_process2_arg1_0) _lh_process2_arg2_0) _lh_process2_arg4_0) _lh_process2_arg5_0) _lh_process2_arg6_0))
and
process_lh__d1 _lh_process_arg1_0 _lh_process_arg2_0 _lh_process_arg3_0 _lh_process_arg4_0 _lh_process_arg5_0 =
  (match _lh_process_arg5_0 with
    | `LH_N -> 
      _lh_process_arg4_0
    | `LH_C(_lh_process_LH_C_0_0, _lh_process_LH_C_1_0) -> 
      (match _lh_process_LH_C_0_0 with
        | `Item(_lh_process_Item_0_0, _lh_process_Item_1_0) -> 
          ((((((process1_lh__d1 _lh_process_arg1_0) _lh_process_arg2_0) _lh_process_arg3_0) _lh_process_arg4_0) _lh_process_LH_C_1_0) ((preprocess_lh__d1 _lh_process_arg4_0) _lh_process_Item_1_0))))
and
split_lh__d1 _lh_split_arg1_0 _lh_split_arg2_0 _lh_process2_arg1_1 _lh_process2_arg2_1 _lh_process2_arg4_1 _lh_process2_arg5_1 _lh_process2_arg6_1 =
  (let rec tmp_0 = ((zip_lh__d1 ((map_lh__d7 _lh_split_arg1_0) _lh_split_arg2_0)) _lh_split_arg2_0) in
    (let rec _lh_process2_LH_P2_1_0 = (let rec _lh_listcomp_fun_3 = (fun _lh_listcomp_fun_para_3 -> 
      (match _lh_listcomp_fun_para_3 with
        | `LH_C(_lh_listcomp_fun_ls_h_3, _lh_listcomp_fun_ls_t_3) -> 
          (match _lh_listcomp_fun_ls_h_3 with
            | `LH_P2(_lh_split_LH_P2_0_0, _lh_split_LH_P2_1_0) -> 
              (if (not _lh_split_LH_P2_0_0) then
                (`LH_C(_lh_split_LH_P2_1_0, (_lh_listcomp_fun_3 _lh_listcomp_fun_ls_t_3)))
              else
                (_lh_listcomp_fun_3 _lh_listcomp_fun_ls_t_3))
            | _ -> 
              (_lh_listcomp_fun_3 _lh_listcomp_fun_ls_t_3))
        | `LH_N -> 
          (`LH_N))) in
      (_lh_listcomp_fun_3 tmp_0)) in
      (let rec _lh_process2_LH_P2_0_0 = (let rec _lh_listcomp_fun_4 = (fun _lh_listcomp_fun_para_4 -> 
        (match _lh_listcomp_fun_para_4 with
          | `LH_C(_lh_listcomp_fun_ls_h_4, _lh_listcomp_fun_ls_t_4) -> 
            (match _lh_listcomp_fun_ls_h_4 with
              | `LH_P2(_lh_split_LH_P2_0_1, _lh_split_LH_P2_1_1) -> 
                (if _lh_split_LH_P2_0_1 then
                  (`LH_C(_lh_split_LH_P2_1_1, (_lh_listcomp_fun_4 _lh_listcomp_fun_ls_t_4)))
                else
                  (_lh_listcomp_fun_4 _lh_listcomp_fun_ls_t_4))
              | _ -> 
                (_lh_listcomp_fun_4 _lh_listcomp_fun_ls_t_4))
          | `LH_N -> 
            (`LH_N))) in
        (_lh_listcomp_fun_4 tmp_0)) in
        (let rec net'_0 = ((add_eqn_lh__d1 ((delete_eqns_lh__d1 ((map_lh__d7 eqno_lh__d1) _lh_process2_LH_P2_0_0)) _lh_process2_arg4_1)) _lh_process2_arg6_1) in
          (let rec deletions_0 = ((map_lh__d7 eqpr_lh__d1) _lh_process2_LH_P2_0_0) in
            (let rec critical_0 = ((all_crit_pairs_lh__d1 _lh_process2_arg6_1) _lh_process2_LH_P2_1_0) in
              (let rec new_agenda_0 = (sift_lh__d1 ((map_lh__d7 (resolve_lh__d1 (super_reduce_lh__d1 net'_0))) ((mappend_lh__d2 deletions_0) critical_0))) in
                (let rec todo'_0 = (((add_agenda_lh__d1 (snd_lh__d1 _lh_process2_arg1_1)) new_agenda_0) _lh_process2_arg5_1) in
                  (((((process_lh__d1 _lh_process2_arg1_1) _lh_process2_arg2_1) ((mappend_lh__d2 _lh_process2_LH_P2_1_0) (`LH_C(_lh_process2_arg6_1, (`LH_N))))) net'_0) todo'_0)))))))));;
let rec knuth_bendix1_lh__d1 _lh_knuth_bendix1_arg1_0 _lh_knuth_bendix1_arg2_0 =
  (((((process_lh__d1 _lh_knuth_bendix1_arg1_0) 1) (`LH_N)) empty_net_lh__d1) (((add_agenda_lh__d1 (snd_lh__d1 _lh_knuth_bendix1_arg1_0)) _lh_knuth_bendix1_arg2_0) (`LH_N)));;
let rec knuth_bendix_lh__d1 _lh_knuth_bendix_arg1_0 _lh_knuth_bendix_arg2_0 =
  ((knuth_bendix1_lh__d1 (`LH_P2(_lh_knuth_bendix_arg1_0, cost_lh__d1))) _lh_knuth_bendix_arg2_0);;
let rec foldr_lh__d5 f_1_5 i_1_4 ls_0 =
  ((ls_0 f_1_5) i_1_4);;
let rec seQ_lh__d1_d1 _lh_seQ_arg1_1 _lh_seQ_arg2_1 =
  ((build_lh__d5 (mk_list_lh__d5 _lh_seQ_arg1_1)) (((foldr_lh__d6 (seq2_lh__d7 mk_cons_lh__d7)) empty_lh__d6) _lh_seQ_arg2_1));;
let rec p_op_lh__d4 _lh_p_op_arg1_2 =
  ((sp_lh__d8 (string_of_lh__d4 opsym_lh__d2)) _lh_p_op_arg1_2);;
let rec q_op_lh__d4 _lh_q_op_arg1_2 =
  (match _lh_q_op_arg1_2 with
    | `LH_C(_lh_q_op_LH_C_0_6, _lh_q_op_LH_C_1_6) -> 
      (match _lh_q_op_LH_C_0_6 with
        | `Expr(_lh_q_op_Expr_0_4) -> 
          (match _lh_q_op_LH_C_1_6 with
            | `LH_C(_lh_q_op_LH_C_0_7, _lh_q_op_LH_C_1_7) -> 
              (match _lh_q_op_LH_C_0_7 with
                | `MkString(_lh_q_op_MkString_0_2) -> 
                  (match _lh_q_op_LH_C_1_7 with
                    | `LH_C(_lh_q_op_LH_C_0_8, _lh_q_op_LH_C_1_8) -> 
                      (match _lh_q_op_LH_C_0_8 with
                        | `Expr(_lh_q_op_Expr_0_5) -> 
                          (match _lh_q_op_LH_C_1_8 with
                            | `LH_N -> 
                              (`Expr((`Func(_lh_q_op_MkString_0_2, (`LH_C(_lh_q_op_Expr_0_4, (`LH_C(_lh_q_op_Expr_0_5, (`LH_N)))))))))
                            | _ -> 
                              (failwith "lh_default_error"))
                        | _ -> 
                          (failwith "lh_default_error"))
                    | _ -> 
                      (failwith "lh_default_error"))
                | _ -> 
                  (failwith "lh_default_error"))
            | _ -> 
              (failwith "lh_default_error"))
        | _ -> 
          (failwith "lh_default_error"))
    | _ -> 
      (failwith "lh_default_error"));;
let rec orElseMap_lh__d1_d1 _lh_orElseMap_arg1_7 _lh_orElseMap_arg2_7 _lh_orElseMap_arg3_7 =
  ((orElse_lh__d6 (_lh_orElseMap_arg1_7 _lh_orElseMap_arg3_7)) (_lh_orElseMap_arg2_7 _lh_orElseMap_arg3_7));;
let rec p_expr_lh__d4 _lh_p_expr_arg1_0 =
  (((orElseMap_lh__d1_d1 ((seQ_lh__d1_d1 q_op_lh__d4) (let rec t_2_5 = (let rec t_2_6 = (let rec t_2_7 = (fun f_3_2 i_3_0 -> 
    i_3_0) in
    (let rec h_2_3 = p_term_lh__d1_d0 in
      (fun f_3_3 i_3_1 -> 
        ((f_3_3 h_2_3) (((foldr_lh__d6 f_3_3) i_3_1) t_2_7))))) in
    (let rec h_2_4 = p_op_lh__d4 in
      (fun f_3_4 i_3_2 -> 
        ((f_3_4 h_2_4) (((foldr_lh__d6 f_3_4) i_3_2) t_2_6))))) in
    (let rec h_2_5 = p_term_lh__d1_d2 in
      (fun f_3_5 i_3_3 -> 
        ((f_3_5 h_2_5) (((foldr_lh__d6 f_3_5) i_3_3) t_2_5))))))) p_term_lh__d1_d1) _lh_p_expr_arg1_0)
and
p_term_lh__d1_d0 _lh_p_term_arg1_6 =
  (((orElseMap_lh__d1_d5 ((seQ_lh__d1_d3 q_func_lh__d6) (let rec t_5_4 = (let rec t_5_5 = (let rec t_5_6 = (let rec t_5_7 = (fun f_6_9 i_6_3 -> 
    i_6_3) in
    (let rec h_5_1 = (look_for_lh__d1_d9 ')') in
      (fun f_7_0 i_6_4 -> 
        ((f_7_0 h_5_1) (((foldr_lh__d6 f_7_0) i_6_4) t_5_7))))) in
    (let rec h_5_2 = ((list_of_lh__d6 p_expr_lh__d4) ',') in
      (fun f_7_1 i_6_5 -> 
        ((f_7_1 h_5_2) (((foldr_lh__d6 f_7_1) i_6_5) t_5_6))))) in
    (let rec h_5_3 = (look_for_lh__d1_d9 '(') in
      (fun f_7_2 i_6_6 -> 
        ((f_7_2 h_5_3) (((foldr_lh__d6 f_7_2) i_6_6) t_5_5))))) in
    (let rec h_5_4 = p_ident_lh__d6 in
      (fun f_7_3 i_6_7 -> 
        ((f_7_3 h_5_4) (((foldr_lh__d6 f_7_3) i_6_7) t_5_4))))))) p_prim_lh__d6) _lh_p_term_arg1_6)
and
p_term_lh__d1_d1 _lh_p_term_arg1_4 =
  (((orElseMap_lh__d1_d5 ((seQ_lh__d1_d3 q_func_lh__d6) (let rec t_3_7 = (let rec t_3_8 = (let rec t_3_9 = (let rec t_4_0 = (fun f_4_8 i_4_2 -> 
    i_4_2) in
    (let rec h_3_5 = (look_for_lh__d1_d9 ')') in
      (fun f_4_9 i_4_3 -> 
        ((f_4_9 h_3_5) (((foldr_lh__d6 f_4_9) i_4_3) t_4_0))))) in
    (let rec h_3_6 = ((list_of_lh__d6 p_expr_lh__d4) ',') in
      (fun f_5_0 i_4_4 -> 
        ((f_5_0 h_3_6) (((foldr_lh__d6 f_5_0) i_4_4) t_3_9))))) in
    (let rec h_3_7 = (look_for_lh__d1_d9 '(') in
      (fun f_5_1 i_4_5 -> 
        ((f_5_1 h_3_7) (((foldr_lh__d6 f_5_1) i_4_5) t_3_8))))) in
    (let rec h_3_8 = p_ident_lh__d6 in
      (fun f_5_2 i_4_6 -> 
        ((f_5_2 h_3_8) (((foldr_lh__d6 f_5_2) i_4_6) t_3_7))))))) p_prim_lh__d6) _lh_p_term_arg1_4)
and
p_term_lh__d1_d2 _lh_p_term_arg1_5 =
  (((orElseMap_lh__d1_d5 ((seQ_lh__d1_d3 q_func_lh__d6) (let rec t_4_7 = (let rec t_4_8 = (let rec t_4_9 = (let rec t_5_0 = (fun f_6_0 i_5_4 -> 
    i_5_4) in
    (let rec h_4_4 = (look_for_lh__d1_d9 ')') in
      (fun f_6_1 i_5_5 -> 
        ((f_6_1 h_4_4) (((foldr_lh__d6 f_6_1) i_5_5) t_5_0))))) in
    (let rec h_4_5 = ((list_of_lh__d6 p_expr_lh__d4) ',') in
      (fun f_6_2 i_5_6 -> 
        ((f_6_2 h_4_5) (((foldr_lh__d6 f_6_2) i_5_6) t_4_9))))) in
    (let rec h_4_6 = (look_for_lh__d1_d9 '(') in
      (fun f_6_3 i_5_7 -> 
        ((f_6_3 h_4_6) (((foldr_lh__d6 f_6_3) i_5_7) t_4_8))))) in
    (let rec h_4_7 = p_ident_lh__d6 in
      (fun f_6_4 i_5_8 -> 
        ((f_6_4 h_4_7) (((foldr_lh__d6 f_6_4) i_5_8) t_4_7))))))) p_prim_lh__d6) _lh_p_term_arg1_5);;
let rec seq2_lh__d3 _lh_seq2_arg1_0 _lh_seq2_arg2_0 _lh_seq2_arg3_0 _lh_funcomp_x_3 =
  (let rec g_0 = (fun xs_1 -> 
    (let rec _lh_matchIdent_0 = xs_1 in
      (match _lh_matchIdent_0 with
        | `LH_P2(_lh_seq2_LH_P2_0_0, _lh_seq2_LH_P2_1_0) -> 
          ((lift_lh__d6 ((cross_lh__d2 (_lh_seq2_arg1_0 _lh_seq2_LH_P2_0_0)) (fun x_7 -> 
            x_7))) (_lh_seq2_arg3_0 _lh_seq2_LH_P2_1_0))))) in
    ((fun _lh_funcomp_x_4 -> 
      (squash_lh__d1 ((lift_lh__d6 g_0) _lh_funcomp_x_4))) (_lh_seq2_arg2_0 _lh_funcomp_x_3)));;
let rec empty_lh__d3 _lh_empty_arg1_2 =
  (succeed_lh__d1 (`LH_P2((`List((`LH_N))), _lh_empty_arg1_2)));;
let rec mk_list_lh__d3 _lh_mk_list_arg1_0 _lh_mk_list_arg2_0 =
  (match _lh_mk_list_arg2_0 with
    | `List(_lh_mk_list_List_0_0) -> 
      (_lh_mk_list_arg1_0 _lh_mk_list_List_0_0)
    | _ -> 
      (failwith "lh_default_error"));;
let rec mk_cons_lh__d3 _lh_mk_cons_arg1_2 _lh_mk_cons_arg2_2 =
  (match _lh_mk_cons_arg2_2 with
    | `List(_lh_mk_cons_List_0_2) -> 
      (`List((`LH_C(_lh_mk_cons_arg1_2, _lh_mk_cons_List_0_2))))
    | _ -> 
      (failwith "lh_default_error"));;
let rec build_lh__d3 _lh_build_arg1_1 _lh_build_arg2_1 _lh_funcomp_x_5 =
  ((lift_lh__d6 ((cross_lh__d2 _lh_build_arg1_1) (fun x_1_0 -> 
    x_1_0))) (_lh_build_arg2_1 _lh_funcomp_x_5));;
let rec seQ_lh__d3 _lh_seQ_arg1_2 _lh_seQ_arg2_2 =
  ((build_lh__d3 (mk_list_lh__d3 _lh_seQ_arg1_2)) (((foldr_lh__d5 (seq2_lh__d3 mk_cons_lh__d3)) empty_lh__d3) _lh_seQ_arg2_2));;
let rec q_eqn_lh__d1 _lh_q_eqn_arg1_0 =
  (match _lh_q_eqn_arg1_0 with
    | `LH_C(_lh_q_eqn_LH_C_0_0, _lh_q_eqn_LH_C_1_0) -> 
      (match _lh_q_eqn_LH_C_1_0 with
        | `LH_C(_lh_q_eqn_LH_C_0_1, _lh_q_eqn_LH_C_1_1) -> 
          (match _lh_q_eqn_LH_C_1_1 with
            | `LH_C(_lh_q_eqn_LH_C_0_2, _lh_q_eqn_LH_C_1_2) -> 
              (match _lh_q_eqn_LH_C_1_2 with
                | `LH_N -> 
                  (`List((`LH_C(_lh_q_eqn_LH_C_0_0, (`LH_C(_lh_q_eqn_LH_C_0_2, (`LH_N)))))))
                | _ -> 
                  (failwith "lh_default_error"))
            | _ -> 
              (failwith "lh_default_error"))
        | _ -> 
          (failwith "lh_default_error"))
    | _ -> 
      (failwith "lh_default_error"));;
let rec orElseMap_lh__d1_d0 _lh_orElseMap_arg1_0 _lh_orElseMap_arg2_0 _lh_orElseMap_arg3_0 =
  ((orElse_lh__d6 (_lh_orElseMap_arg1_0 _lh_orElseMap_arg3_0)) (_lh_orElseMap_arg2_0 _lh_orElseMap_arg3_0));;
let rec p_op_lh__d3 _lh_p_op_arg1_1 =
  ((sp_lh__d8 (string_of_lh__d4 opsym_lh__d2)) _lh_p_op_arg1_1);;
let rec seQ_lh__d1_d0 _lh_seQ_arg1_3 _lh_seQ_arg2_3 =
  ((build_lh__d5 (mk_list_lh__d5 _lh_seQ_arg1_3)) (((foldr_lh__d6 (seq2_lh__d7 mk_cons_lh__d7)) empty_lh__d6) _lh_seQ_arg2_3));;
let rec q_op_lh__d3 _lh_q_op_arg1_1 =
  (match _lh_q_op_arg1_1 with
    | `LH_C(_lh_q_op_LH_C_0_3, _lh_q_op_LH_C_1_3) -> 
      (match _lh_q_op_LH_C_0_3 with
        | `Expr(_lh_q_op_Expr_0_2) -> 
          (match _lh_q_op_LH_C_1_3 with
            | `LH_C(_lh_q_op_LH_C_0_4, _lh_q_op_LH_C_1_4) -> 
              (match _lh_q_op_LH_C_0_4 with
                | `MkString(_lh_q_op_MkString_0_1) -> 
                  (match _lh_q_op_LH_C_1_4 with
                    | `LH_C(_lh_q_op_LH_C_0_5, _lh_q_op_LH_C_1_5) -> 
                      (match _lh_q_op_LH_C_0_5 with
                        | `Expr(_lh_q_op_Expr_0_3) -> 
                          (match _lh_q_op_LH_C_1_5 with
                            | `LH_N -> 
                              (`Expr((`Func(_lh_q_op_MkString_0_1, (`LH_C(_lh_q_op_Expr_0_2, (`LH_C(_lh_q_op_Expr_0_3, (`LH_N)))))))))
                            | _ -> 
                              (failwith "lh_default_error"))
                        | _ -> 
                          (failwith "lh_default_error"))
                    | _ -> 
                      (failwith "lh_default_error"))
                | _ -> 
                  (failwith "lh_default_error"))
            | _ -> 
              (failwith "lh_default_error"))
        | _ -> 
          (failwith "lh_default_error"))
    | _ -> 
      (failwith "lh_default_error"));;
let rec p_expr_lh__d3 _lh_p_expr_arg1_3 =
  (((orElseMap_lh__d1_d0 ((seQ_lh__d1_d0 q_op_lh__d3) (let rec t_7_7 = (let rec t_7_8 = (let rec t_7_9 = (fun f_9_8 i_8_9 -> 
    i_8_9) in
    (let rec h_7_4 = p_term_lh__d8 in
      (fun f_9_9 i_9_0 -> 
        ((f_9_9 h_7_4) (((foldr_lh__d6 f_9_9) i_9_0) t_7_9))))) in
    (let rec h_7_5 = p_op_lh__d3 in
      (fun f_1_0_0 i_9_1 -> 
        ((f_1_0_0 h_7_5) (((foldr_lh__d6 f_1_0_0) i_9_1) t_7_8))))) in
    (let rec h_7_6 = p_term_lh__d9 in
      (fun f_1_0_1 i_9_2 -> 
        ((f_1_0_1 h_7_6) (((foldr_lh__d6 f_1_0_1) i_9_2) t_7_7))))))) p_term_lh__d7) _lh_p_expr_arg1_3)
and
p_term_lh__d7 _lh_p_term_arg1_3 =
  (((orElseMap_lh__d1_d5 ((seQ_lh__d1_d3 q_func_lh__d6) (let rec t_2_1 = (let rec t_2_2 = (let rec t_2_3 = (let rec t_2_4 = (fun f_2_7 i_2_5 -> 
    i_2_5) in
    (let rec h_1_9 = (look_for_lh__d1_d9 ')') in
      (fun f_2_8 i_2_6 -> 
        ((f_2_8 h_1_9) (((foldr_lh__d6 f_2_8) i_2_6) t_2_4))))) in
    (let rec h_2_0 = ((list_of_lh__d6 p_expr_lh__d3) ',') in
      (fun f_2_9 i_2_7 -> 
        ((f_2_9 h_2_0) (((foldr_lh__d6 f_2_9) i_2_7) t_2_3))))) in
    (let rec h_2_1 = (look_for_lh__d1_d9 '(') in
      (fun f_3_0 i_2_8 -> 
        ((f_3_0 h_2_1) (((foldr_lh__d6 f_3_0) i_2_8) t_2_2))))) in
    (let rec h_2_2 = p_ident_lh__d6 in
      (fun f_3_1 i_2_9 -> 
        ((f_3_1 h_2_2) (((foldr_lh__d6 f_3_1) i_2_9) t_2_1))))))) p_prim_lh__d6) _lh_p_term_arg1_3)
and
p_term_lh__d8 _lh_p_term_arg1_9 =
  (((orElseMap_lh__d1_d5 ((seQ_lh__d1_d3 q_func_lh__d6) (let rec t_8_0 = (let rec t_8_1 = (let rec t_8_2 = (let rec t_8_3 = (fun f_1_0_2 i_9_3 -> 
    i_9_3) in
    (let rec h_7_7 = (look_for_lh__d1_d9 ')') in
      (fun f_1_0_3 i_9_4 -> 
        ((f_1_0_3 h_7_7) (((foldr_lh__d6 f_1_0_3) i_9_4) t_8_3))))) in
    (let rec h_7_8 = ((list_of_lh__d6 p_expr_lh__d3) ',') in
      (fun f_1_0_4 i_9_5 -> 
        ((f_1_0_4 h_7_8) (((foldr_lh__d6 f_1_0_4) i_9_5) t_8_2))))) in
    (let rec h_7_9 = (look_for_lh__d1_d9 '(') in
      (fun f_1_0_5 i_9_6 -> 
        ((f_1_0_5 h_7_9) (((foldr_lh__d6 f_1_0_5) i_9_6) t_8_1))))) in
    (let rec h_8_0 = p_ident_lh__d6 in
      (fun f_1_0_6 i_9_7 -> 
        ((f_1_0_6 h_8_0) (((foldr_lh__d6 f_1_0_6) i_9_7) t_8_0))))))) p_prim_lh__d6) _lh_p_term_arg1_9)
and
p_term_lh__d9 _lh_p_term_arg1_1_0 =
  (((orElseMap_lh__d1_d5 ((seQ_lh__d1_d3 q_func_lh__d6) (let rec t_9_3 = (let rec t_9_4 = (let rec t_9_5 = (let rec t_9_6 = (fun f_1_1_7 i_1_0_4 -> 
    i_1_0_4) in
    (let rec h_9_0 = (look_for_lh__d1_d9 ')') in
      (fun f_1_1_8 i_1_0_5 -> 
        ((f_1_1_8 h_9_0) (((foldr_lh__d6 f_1_1_8) i_1_0_5) t_9_6))))) in
    (let rec h_9_1 = ((list_of_lh__d6 p_expr_lh__d3) ',') in
      (fun f_1_1_9 i_1_0_6 -> 
        ((f_1_1_9 h_9_1) (((foldr_lh__d6 f_1_1_9) i_1_0_6) t_9_5))))) in
    (let rec h_9_2 = (look_for_lh__d1_d9 '(') in
      (fun f_1_2_0 i_1_0_7 -> 
        ((f_1_2_0 h_9_2) (((foldr_lh__d6 f_1_2_0) i_1_0_7) t_9_4))))) in
    (let rec h_9_3 = p_ident_lh__d6 in
      (fun f_1_2_1 i_1_0_8 -> 
        ((f_1_2_1 h_9_3) (((foldr_lh__d6 f_1_2_1) i_1_0_8) t_9_3))))))) p_prim_lh__d6) _lh_p_term_arg1_1_0);;
let rec sp_lh__d3 _lh_sp_arg1_1 _lh_funcomp_x_2 =
  (_lh_sp_arg1_1 ((dropWhile_lh__d1 (fun x_6 -> 
    (x_6 = ' '))) _lh_funcomp_x_2));;
let rec pchar_lh__d1 _lh_pchar_arg1_3 _lh_pchar_arg2_3 =
  (match _lh_pchar_arg2_3 with
    | `LH_N -> 
      croak_lh__d1
    | `LH_C(_lh_pchar_LH_C_0_3, _lh_pchar_LH_C_1_3) -> 
      (if (_lh_pchar_arg1_3 _lh_pchar_LH_C_0_3) then
        (succeed_lh__d1 (`LH_P2((`MkString((`LH_C(_lh_pchar_LH_C_0_3, (`LH_N))))), _lh_pchar_LH_C_1_3)))
      else
        croak_lh__d1));;
let rec look_for_lh__d1 _lh_look_for_arg1_1_2 =
  (sp_lh__d3 (pchar_lh__d1 (fun x_5_3 -> 
    (x_5_3 = _lh_look_for_arg1_1_2))));;
let rec p_eqn_lh__d1 _lh_p_eqn_arg1_0 =
  (((seQ_lh__d3 q_eqn_lh__d1) (let rec t_4_4 = (let rec t_4_5 = (let rec t_4_6 = (fun f_5_5 i_4_9 -> 
    i_4_9) in
    (let rec h_4_1 = p_expr_lh__d3 in
      (fun f_5_6 i_5_0 -> 
        ((f_5_6 h_4_1) (((foldr_lh__d5 f_5_6) i_5_0) t_4_6))))) in
    (let rec h_4_2 = (look_for_lh__d1 '=') in
      (fun f_5_7 i_5_1 -> 
        ((f_5_7 h_4_2) (((foldr_lh__d5 f_5_7) i_5_1) t_4_5))))) in
    (let rec h_4_3 = p_expr_lh__d4 in
      (fun f_5_8 i_5_2 -> 
        ((f_5_8 h_4_3) (((foldr_lh__d5 f_5_8) i_5_2) t_4_4)))))) _lh_p_eqn_arg1_0);;
let rec parse_eqn_lh__d1 _lh_parse_eqn_arg1_0 =
  (let rec _lh_matchIdent_1_5 = (fst_lh__d3 (the_lh__d3 (p_eqn_lh__d1 _lh_parse_eqn_arg1_0))) in
    (match _lh_matchIdent_1_5 with
      | `List(_lh_parse_eqn_List_0_0) -> 
        (match _lh_parse_eqn_List_0_0 with
          | `LH_C(_lh_parse_eqn_LH_C_0_0, _lh_parse_eqn_LH_C_1_0) -> 
            (match _lh_parse_eqn_LH_C_0_0 with
              | `Expr(_lh_parse_eqn_Expr_0_0) -> 
                (match _lh_parse_eqn_LH_C_1_0 with
                  | `LH_C(_lh_parse_eqn_LH_C_0_1, _lh_parse_eqn_LH_C_1_1) -> 
                    (match _lh_parse_eqn_LH_C_0_1 with
                      | `Expr(_lh_parse_eqn_Expr_0_1) -> 
                        (match _lh_parse_eqn_LH_C_1_1 with
                          | `LH_N -> 
                            (`LH_P2(_lh_parse_eqn_Expr_0_0, _lh_parse_eqn_Expr_0_1))
                          | _ -> 
                            (failwith "lh_default_error"))
                      | _ -> 
                        (failwith "lh_default_error"))
                  | _ -> 
                    (failwith "lh_default_error"))
              | _ -> 
                (failwith "lh_default_error"))
          | _ -> 
            (failwith "lh_default_error"))
      | _ -> 
        (failwith "lh_default_error")));;
let rec testRewrite_nofib_lh__d1 _lh_testRewrite_nofib_arg1_0 =
  ((all_lh__d1 result_lh__d1) ((copy_lh__d1 _lh_testRewrite_nofib_arg1_0) (let rec _lh_result_LH_P2_1_0 = (`LH_C('I', (`LH_C('(', (`LH_C('b', (`LH_C(')', (`LH_C(' ', (`LH_C('*', (`LH_C(' ', (`LH_C('I', (`LH_C('(', (`LH_C('a', (`LH_C(')', (`LH_N))))))))))))))))))))))) in
    (let rec _lh_result_LH_P2_0_0 = (`LH_C('I', (`LH_C('(', (`LH_C('a', (`LH_C(' ', (`LH_C('*', (`LH_C(' ', (`LH_C('b', (`LH_C(')', (`LH_N))))))))))))))))) in
      (fun _lh_dummy_0 -> 
        (let rec group_completion_0 = ((knuth_bendix_lh__d1 ((rpo_lh__d1 (rank_order_lh__d1 rank_lh__d1)) lex_ext_lh__d1)) ((map_lh__d1 parse_eqn_lh__d1) (let rec t_2_8 = (let rec t_2_9 = (let rec t_3_0 = (fun f_3_6 -> 
          (`LH_N)) in
          (let rec h_2_6 = (`LH_C('I', (`LH_C('(', (`LH_C('x', (`LH_C(')', (`LH_C(' ', (`LH_C('*', (`LH_C(' ', (`LH_C('x', (`LH_C(' ', (`LH_C('=', (`LH_C(' ', (`LH_C('E', (`LH_N))))))))))))))))))))))))) in
            (fun f_3_7 -> 
              (`LH_C((f_3_7 h_2_6), ((map_lh__d1 f_3_7) t_3_0)))))) in
          (let rec h_2_7 = (`LH_C('E', (`LH_C(' ', (`LH_C('*', (`LH_C(' ', (`LH_C('x', (`LH_C(' ', (`LH_C('=', (`LH_C(' ', (`LH_C('x', (`LH_N))))))))))))))))))) in
            (fun f_3_8 -> 
              (`LH_C((f_3_8 h_2_7), ((map_lh__d1 f_3_8) t_2_9)))))) in
          (let rec h_2_8 = (`LH_C('(', (`LH_C('a', (`LH_C(' ', (`LH_C('*', (`LH_C(' ', (`LH_C('b', (`LH_C(')', (`LH_C(' ', (`LH_C('*', (`LH_C(' ', (`LH_C('c', (`LH_C(' ', (`LH_C('=', (`LH_C(' ', (`LH_C('a', (`LH_C(' ', (`LH_C('*', (`LH_C(' ', (`LH_C('(', (`LH_C('b', (`LH_C(' ', (`LH_C('*', (`LH_C(' ', (`LH_C('c', (`LH_C(')', (`LH_N))))))))))))))))))))))))))))))))))))))))))))))))))) in
            (fun f_3_9 -> 
              (`LH_C((f_3_9 h_2_8), ((map_lh__d1 f_3_9) t_2_8)))))))) in
          ((eqExpr_lh__d1 ((simplify_lh__d1 (super_reduce_lh__d1 group_completion_0)) (parse_lh__d1 _lh_result_LH_P2_0_0))) (parse_lh__d2 _lh_result_LH_P2_1_0))))))));;
let run () = 1 + (Obj.magic ((testRewrite_nofib_lh__d1 50)));
end;;

