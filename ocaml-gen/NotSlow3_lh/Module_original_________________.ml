...
let rec concat_lh lss_0 =
  (match lss_0 with
    ...
    | `LH_N -> 
      (`LH_N));;
let rec testNotSlow3_lh _lh_testNotSlow3_arg1_0 =
  (if (_lh_testNotSlow3_arg1_0 < 0) then
    (`LH_N)
  else
    (let rec tmp_0 = (testNotSlow3_lh (_lh_testNotSlow3_arg1_0 - 1)) in
      (concat_lh (let rec _lh_listcomp_fun_0 = (fun _lh_listcomp_fun_para_0 -> 
        (match _lh_listcomp_fun_para_0 with
          ...
          | `LH_N -> 
            (`LH_N))) in
        (_lh_listcomp_fun_0 ((enumFromTo_lh 1) _lh_testNotSlow3_arg1_0))))));;
let run () = 1 + (Obj.magic ((testNotSlow3_lh 10)));
end;;
