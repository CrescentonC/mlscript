  ...
  (if (a_0 <= b_0) then
    (let rec _lh_listcomp_fun_ls_t_0 = ((enumFromTo_lh__d1 (a_0 + 1)) b_0) in
      (let rec _lh_listcomp_fun_ls_h_0 = a_0 in
        (fun _lh_listcomp_fun_0 _lh_testSlow_arg1_0 -> 
          (let rec t_1 = (_lh_listcomp_fun_0 _lh_listcomp_fun_ls_t_0) in
            (let rec h_1 = (testSlow_lh__d1 (_lh_testSlow_arg1_0 - 1)) in
              (fun _lh_dummy_0 -> 
                ((mappend_lh__d1 h_1) (concat_lh__d1 t_1))))))))
  else
    (fun _lh_listcomp_fun_1 _lh_testSlow_arg1_1 _lh_dummy_1 -> 
      (`LH_N)))
and
testSlow_lh__d1 _lh_testSlow_arg1_2 =
  (if (_lh_testSlow_arg1_2 < 0) then
    (`LH_N)
  else
    (concat_lh__d1 (let rec _lh_listcomp_fun_2 = (fun _lh_listcomp_fun_para_0 -> 
      ((_lh_listcomp_fun_para_0 _lh_listcomp_fun_2) _lh_testSlow_arg1_2)) in
      (_lh_listcomp_fun_2 ((enumFromTo_lh__d1 1) _lh_testSlow_arg1_2)))));;
let run () = 1 + (Obj.magic ((testSlow_lh__d1 10)));
end;;
