...
let rec f_lh _lh_f_arg1_0 _lh_f_arg2_0 =
  ((map_lh (fun x_0 -> 
    (testNotSlow_lh (_lh_f_arg1_0 - 1)))) _lh_f_arg2_0)
and
testNotSlow_lh _lh_testNotSlow_arg1_0 =
  (if (_lh_testNotSlow_arg1_0 < 0) then
    (`LH_N)
  else
    (concat_lh ((f_lh _lh_testNotSlow_arg1_0) ((enumFromTo_lh 1) _lh_testNotSlow_arg1_0))));;
let run () = 1 + (Obj.magic ((testNotSlow_lh 10)));
end;;
