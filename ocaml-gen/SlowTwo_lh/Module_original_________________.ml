...
let rec enumFromTo_lh a_0 b_0 =
  (if (a_0 <= b_0) then
    ...
    (`LH_N));;
let rec testSlowTwo_lh _lh_testSlowTwo_arg1_0 =
    ...
    (concat_lh (let rec _lh_listcomp_fun_0 = (fun _lh_listcomp_fun_para_0 -> 
      (match _lh_listcomp_fun_para_0 with
        | `LH_C(_lh_listcomp_fun_ls_h_0, _lh_listcomp_fun_ls_t_0) -> 
          (`LH_C((testSlowTwo_lh (_lh_testSlowTwo_arg1_0 - 1)), (_lh_listcomp_fun_0 _lh_listcomp_fun_ls_t_0)))
        | `LH_N -> 
          (`LH_N))) in
      (_lh_listcomp_fun_0 ((enumFromTo_lh 0) _lh_testSlowTwo_arg1_0)))));;
let run () = 1 + (Obj.magic ((testSlowTwo_lh 8)));
end;;
